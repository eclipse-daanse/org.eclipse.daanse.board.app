...
/** An MDX query supports up to 128 axes, numbered 0 through 127. */
const MAX_AXIS = 127;

/** Identifiers must contain from 1 through 100 characters. */
const MAX_IDENTIFIER_LENGTH = 100;

/** Permissible SOLVE_ORDER range; negative indices are reserved by the engine. */
const MIN_SOLVE_ORDER = -8181;
const MAX_SOLVE_ORDER = 65535;

/** Reserved keywords that must be bracketed when used as object names. */
const RESERVED_KEYWORDS = new Set([
    'ABSOLUTE', 'ADDCALCULATEDMEMBERS', 'AFTER', 'AGGREGATE', 'ALL', 'AND', 'AS', 'ASC',
    'AXIS', 'BASC', 'BDESC', 'BEFORE', 'BEFORE_AND_AFTER', 'BOTTOMCOUNT', 'BOTTOMPERCENT',
    'BOTTOMSUM', 'BY', 'CALCULATE', 'CALCULATED', 'CALCULATION', 'CALL', 'CAPTION', 'CASE',
    'CAST', 'CELL', 'CHAPTERS', 'CHILDREN', 'CLOSINGPERIOD', 'COLUMNS', 'COUSIN', 'CREATE',
    'CUBE', 'CURRENTCUBE', 'CURRENTMEMBER', 'DESC', 'DESCENDANTS', 'DIMENSION', 'DISTINCT',
    'DRILLDOWNLEVEL', 'DRILLTHROUGH', 'DROP', 'ELSE', 'EMPTY', 'END', 'EXISTING', 'EXPLAIN',
    'FILTER', 'FIRSTROWSET', 'FREEZE', 'FROM', 'GLOBAL', 'HIDDEN', 'HIERARCHY', 'IS',
    'LEAVES', 'LEVEL', 'MAXROWS', 'MEASURE', 'MEMBER', 'MEMBERS', 'NON', 'NOT', 'NULL',
    'ON', 'OR', 'ORDER', 'PAGES', 'PARENT', 'PLAN', 'PROPERTIES', 'RECURSIVE', 'REFRESH',
    'RETURN', 'ROWS', 'SCOPE', 'SECTIONS', 'SELECT', 'SELF', 'SELF_AND_AFTER',
    'SELF_AND_BEFORE', 'SELF_BEFORE_AFTER', 'SESSION', 'SET', 'SOLVE_ORDER', 'SUBCUBE',
    'THEN', 'THIS', 'TYPED', 'UNION', 'UPDATE', 'USE_EQUAL_ALLOCATION',
    'USE_EQUAL_INCREMENT', 'USE_WEIGHTED_ALLOCATION', 'USE_WEIGHTED_INCREMENT', 'VISUAL',
    'VISUALTOTALS', 'WHEN', 'WHERE', 'WITH', 'XOR'
]);

export function registerValidationChecks(services: { validation: { ValidationRegistry: { register(checks: ValidationChecks<MdxAstType>, thisObj: unknown): void } } }): void {
    const validator = new MdxValidator();
    const checks: ValidationChecks<MdxAstType> = {
        SelectStatement: [validator.checkAxisOrdinals, validator.checkSlicerIsNotASet],
        AxisClause: validator.checkAxisTargetIsInteger,
        CellPropertyList: validator.checkCellProperties,
        CreateStatement: [validator.checkCreateTargetsCurrentCube, validator.checkDeclarationName],
        WithMember: validator.checkDeclarationName,
        WithSet: validator.checkDeclarationName,
        PropertyAssignment: [validator.checkKnownProperty, validator.checkSolveOrder, validator.checkScopeIsolation],
...
export class MdxValidator {
    /**
     * Axes must be numbered 0..127 with no duplicates and no gaps: a query
     * that skips an axis cannot be projected onto a result grid.
     */
    checkAxisOrdinals = (select: SelectStatement, accept: ValidationAcceptor): void => {
        ...
        select.axes.forEach((axis, index) => {
            ...
            if (ordinal > MAX_AXIS || ordinal < 0) {
                accept('error', `Axis ordinal must be between 0 and ${MAX_AXIS}, but was ${ordinal}.`, {
                    node: axis,
                    property: 'target'
                });
                return;
    ...
    /** An axis given by expression must be a non-negative integer literal. */
    checkAxisTargetIsInteger = (axis: AxisClause, accept: ValidationAcceptor): void => {
        const target = axis.target;
        if (!target || (target.$type !== 'OrdinalAxis' && target.$type !== 'AxisFunction')) {
            return;
        }
     ...
     */
    checkSlicerIsNotASet = (select: SelectStatement, accept: ValidationAcceptor): void => {
    ...
    checkCellProperties = (list: CellPropertyList, accept: ValidationAcceptor): void => {
    ...
    /**
     * Naming a cube other than the one currently connected is an error at
     * execution time, so CREATE statements should target CURRENTCUBE.
     */
    checkCreateTargetsCurrentCube = (statement: CreateStatement, accept: ValidationAcceptor): void => {
    ...
    checkKnownProperty = (assignment: PropertyAssignment, accept: ValidationAcceptor): void => {
        const name = (assignment.name ?? '').toUpperCase();
        if (DEPRECATED_MEMBER_PROPERTIES.has(name)) {
            accept('warning', `The '${assignment.name}' property is deprecated; avoid setting it.`, {
                node: assignment,
                property: 'name'
            });
            return;
        ...
        if (!CALCULATED_MEMBER_PROPERTIES.has(name)) {
            accept('warning', `Unknown calculated member property '${assignment.name}'.`, {
                node: assignment,
                property: 'name'
            });
        }
    ...
    /**
     * The engine reserves negative solve orders (Custom Rollup uses -5119),
     * so developers should stay in the positive range.
     */
    checkSolveOrder = (assignment: PropertyAssignment, accept: ValidationAcceptor): void => {
    ...
    checkScopeIsolation = (assignment: PropertyAssignment, accept: ValidationAcceptor): void => {
    ...
    checkFunctionCall = (call: FunctionCall, accept: ValidationAcceptor): void => {
        ...
        if (!signature) {
            accept('warning', `Unknown MDX function '${call.name}'.`, { node: call, property: 'name' });
            return;
        }
    ...
    /**
     * Both regular and delimited identifiers must contain 1 to 100 characters,
     * and a reserved keyword used as a bare name needs bracketing.
     */
...
/**
 * The declared or implied ordinal of an axis. Named axes map to their
 * conventional numbers; an expression must be a numeric literal to be known
 * statically. Returns undefined when the ordinal cannot be determined.
 */
function axisOrdinal(axis: AxisClause, _position: number): number | undefined {
    ...
    switch (target.$type) {
        case 'NamedAxis':
            return ['COLUMNS', 'ROWS', 'PAGES', 'SECTIONS', 'CHAPTERS'].indexOf((target.name ?? '').toUpperCase());
        case 'AxisFunction':
        case 'OrdinalAxis':
...
/** The value of an expression when it is a (possibly negated) numeric literal. */
function constantNumber(expression: Expression | undefined): number | undefined {
    if (!expression) {
        return undefined;
    }
    ...
    if (isUnaryExpression(expression) && (expression.operator === '-' || expression.operator === '+')) {
        const inner = constantNumber(expression.operand);
        return inner === undefined ? undefined : expression.operator === '-' ? -inner : inner;
    }
    return undefined;
}
