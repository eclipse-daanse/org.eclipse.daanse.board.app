/**
 * Signatures for MDX intrinsic functions.
 *
 * The grammar parses function calls generically (`Name(args)`), so name and
 * arity checking happens here instead. `maxArgs: undefined` means variadic.
 *
 * Seeded from the functions named in MDX-specs.md and extended from the
 * Microsoft MDX function reference. Names are compared case-insensitively.
 */
...
export interface MdxFunctionSignature {
    ...
    /** Undefined means the function is variadic. */
    readonly maxArgs?: number;
    readonly category: MdxFunctionCategory;
    /** Set for functions Microsoft advises against; surfaced as a warning. */
...
const DISPLAY_NAMES: Record<string, string> = {
    AGGREGATE: 'Aggregate', SUM: 'Sum', COUNT: 'Count', DISTINCTCOUNT: 'DistinctCount',
    AVG: 'Avg', MAX: 'Max', MIN: 'Min', MEDIAN: 'Median',

    STDEV: 'Stdev', STDEVP: 'StdevP', VAR: 'Var', VARP: 'VarP',
    COVARIANCE: 'Covariance', COVARIANCEN: 'CovarianceN', CORRELATION: 'Correlation',
    LINREGINTERCEPT: 'LinRegIntercept', LINREGSLOPE: 'LinRegSlope', LINREGR2: 'LinRegR2',
    LINREGPOINT: 'LinRegPoint', LINREGVARIANCE: 'LinRegVariance', RANK: 'Rank',

    ANCESTOR: 'Ancestor', ANCESTORS: 'Ancestors', CHILDREN: 'Children', COUSIN: 'Cousin',
    FIRSTCHILD: 'FirstChild', FIRSTSIBLING: 'FirstSibling', LASTCHILD: 'LastChild',
    LASTSIBLING: 'LastSibling', LEAD: 'Lead', LAG: 'Lag', NEXTMEMBER: 'NextMember',
    PREVMEMBER: 'PrevMember', PARENT: 'Parent', SIBLINGS: 'Siblings',

    CROSSJOIN: 'Crossjoin', NONEMPTYCROSSJOIN: 'NonEmptyCrossjoin', UNION: 'Union',
    INTERSECT: 'Intersect', EXCEPT: 'Except', EXISTS: 'Exists', DISTINCT: 'Distinct',
    FILTER: 'Filter', GENERATE: 'Generate', HEAD: 'Head', TAIL: 'Tail', SUBSET: 'Subset',
    TOPCOUNT: 'TopCount', BOTTOMCOUNT: 'BottomCount', TOPPERCENT: 'TopPercent',
    BOTTOMPERCENT: 'BottomPercent', TOPSUM: 'TopSum', BOTTOMSUM: 'BottomSum',
    HIERARCHIZE: 'Hierarchize', MEMBERS: 'Members', ADDCALCULATEDMEMBERS: 'AddCalculatedMembers',
    STRIPCALCULATEDMEMBERS: 'StripCalculatedMembers', VISUALTOTALS: 'VisualTotals',
    UNORDER: 'Unorder', EXTRACT: 'Extract',

    DRILLDOWNLEVEL: 'DrilldownLevel', DRILLUPLEVEL: 'DrillupLevel',
    DRILLDOWNMEMBER: 'DrilldownMember', DRILLUPMEMBER: 'DrillupMember',
    EXISTING: 'Existing', ROLLUPCHILDREN: 'RollupChildren',

    CURRENTMEMBER: 'CurrentMember', DEFAULTMEMBER: 'DefaultMember', DATAMEMBER: 'DataMember',
    STRTOMEMBER: 'StrToMember', STRTOSET: 'StrToSet', STRTOTUPLE: 'StrToTuple',
    STRTOVALUE: 'StrToValue', ITEM: 'Item', VALIDMEASURE: 'ValidMeasure',
    UNKNOWNMEMBER: 'UnknownMember',

    PROPERTIES: 'Properties', NAME: 'Name', UNIQUENAME: 'UniqueName', LEVEL: 'Level',
    LEVELS: 'Levels', ORDINAL: 'Ordinal', DIMENSION: 'Dimension', HIERARCHY: 'Hierarchy',
    VALUE: 'Value',

    PARALLELPERIOD: 'ParallelPeriod', PERIODSTODATE: 'PeriodsToDate',
    OPENINGPERIOD: 'OpeningPeriod', CLOSINGPERIOD: 'ClosingPeriod', LASTPERIODS: 'LastPeriods',
    MTD: 'Mtd', QTD: 'Qtd', YTD: 'Ytd', WTD: 'Wtd',

    KPIVALUE: 'KpiValue', KPIGOAL: 'KpiGoal', KPISTATUS: 'KpiStatus', KPITREND: 'KpiTrend',
    KPIWEIGHT: 'KpiWeight', KPICURRENTTIMEMEMBER: 'KpiCurrentTimeMember',

    CALCULATIONCURRENTPASS: 'CalculationCurrentPass', CALCULATIONPASSVALUE: 'CalculationPassValue',
    ROOT: 'Root', LEAVES: 'Leaves', THIS: 'This', LOOKUPCUBE: 'LookupCube',

    IIF: 'IIf', ISEMPTY: 'IsEmpty', ISANCESTOR: 'IsAncestor', ISGENERATION: 'IsGeneration',
    ISLEAF: 'IsLeaf', ISSIBLING: 'IsSibling', COALESCEEMPTY: 'CoalesceEmpty',

    ABS: 'Abs', ROUND: 'Round', INT: 'Int', SIGN: 'Sign', FORMAT: 'Format',
    CAPTION: 'Caption', GENERATESTRING: 'GenerateString', SETTOSTR: 'SetToStr',
    TUPLETOSTR: 'TupleToStr', MEMBERTOSTR: 'MemberToStr', USERNAME: 'UserName',
    CUSTOMDATA: 'CustomData', ERROR: 'Error',

    DESCENDANTS: 'Descendants', ORDER: 'Order'
};

/** Canonical spelling of a function name; the input itself if unknown. */
export function functionDisplayName(name: string): string {
    return DISPLAY_NAMES[name.toUpperCase()] ?? name;
}

/**
 * Intrinsic member properties (MDX-specs.md, "Extracting Member Properties").
 * Used to distinguish a metadata accessor from an ordinary member segment.
 */
...
/** Cell properties valid in a CELL PROPERTIES clause. */
...
/** Properties accepted in a calculated-member definition. */
...
/** Deprecated calculated-member properties. */
