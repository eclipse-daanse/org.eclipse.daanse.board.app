...
class MemberPathScope implements Scope {
  ...
  getElement(name: string): AstNodeDescription | undefined {
    return (
      this.elements.get(normalizeMemberPath(name)) ??
      this.outerScope?.getElement(name)
    )
  }
  ...
  getElements(name: string): Stream<AstNodeDescription> {
    ...
    const outer =
      this.outerScope?.getElements(name) ?? stream<AstNodeDescription>([])
...
export class MdxScopeProvider extends DefaultScopeProvider {
  override getScope(context: ReferenceInfo): Scope {
    ...
    for (const declaration of this.visibleDeclarations(context.container)) {
      ...
      if (last && last !== name) {
        descriptions.push(
          this.descriptions.createDescription(declaration, last),
        )
      }
    }
