...
const KEYWORDS = new Set(
    [
        'SELECT', 'FROM', 'WHERE', 'WITH', 'ON', 'AS', 'NON', 'EMPTY', 'VISUAL',
        'CREATE', 'DROP', 'SESSION', 'GLOBAL', 'HIDDEN', 'CALCULATED', 'MEMBER',
        'SET', 'SUBCUBE', 'KPI', 'MEASURE', 'CELL', 'CALCULATION', 'DIMENSION',
        'PROPERTIES', 'SCOPE', 'END', 'CALCULATE', 'FREEZE', 'CASE', 'WHEN',
        'THEN', 'ELSE', 'FOR', 'CAST', 'DRILLTHROUGH', 'MAXROWS', 'FIRSTROWSET',
        'RETURN', 'EXPLAIN', 'PLAN', 'REFRESH', 'CUBE', 'UPDATE', 'BY', 'CALL'
    ].map((k) => k.toLowerCase())
);

/** Operators spelled as words. `IS` compares object identity, not value. */
const WORD_OPERATORS = new Set(['and', 'or', 'not', 'xor', 'is', 'existing']);

/**
 * Axis names, the positional flags of Descendants/Order/.Properties, and the
 * writeback allocation modes of UPDATE CUBE.
 */
const LANGUAGE_CONSTANTS = new Set(
    [
        'COLUMNS', 'ROWS', 'PAGES', 'SECTIONS', 'CHAPTERS', 'AXIS',
        'SELF_AND_AFTER', 'SELF_AND_BEFORE', 'SELF_BEFORE_AFTER', 'SELF',
        'AFTER', 'BEFORE', 'LEAVES', 'BASC', 'BDESC', 'ASC', 'DESC', 'TYPED',
        'NULL', 'USE_EQUAL_ALLOCATION', 'USE_EQUAL_INCREMENT',
        'USE_WEIGHTED_ALLOCATION', 'USE_WEIGHTED_INCREMENT'
    ].map((k) => k.toLowerCase())
);

/** Contextual references to the current cube and subcube. */
const CONTEXT_KEYWORDS = new Set(['currentcube', 'this']);

/** Calculated-member properties, which appear as `NAME = value`. */
const MEMBER_PROPERTIES = new Set(
    [
        'SOLVE_ORDER', 'FORMAT_STRING', 'VISIBLE', 'NON_EMPTY_BEHAVIOR', 'CAPTION',
        'DISPLAY_FOLDER', 'ASSOCIATED_MEASURE_GROUP', 'SCOPE_ISOLATION', 'BACK_COLOR',
        'FORE_COLOR', 'FONT_NAME', 'FONT_SIZE', 'FONT_FLAGS', 'GOAL', 'STATUS',
        'TREND', 'WEIGHT'
    ].map((k) => k.toLowerCase())
);
...
interface MdxState {
    ...
    /** The quote character of an unterminated string, or null. */
    inString: string | null;
    /** True when the previous token was `.`, so this one is a property. */
    afterDot: boolean;
}
...
const mdxStreamParser: StreamParser<MdxState> = {
    ...
    token(stream, state) {
        ...
        if (IDENTIFIER_START.test(ch)) {
            ...
            if (MEMBER_PROPERTIES.has(word) && /^\s*=/.test(rest)) return 'propertyName';
            return null;
        }
...
/** Consumes a `]`-terminated body, treating `]]` as an escaped `]`. */
/**
 * Consumes a string body and reports the state to carry to the next line.
 *
 * Always null: MDX strings do not span lines, so an unterminated one is closed
 * at end-of-line rather than bleeding the colour into the rest of the
 * document. It returns a value rather than assigning `null` at each call site
 * so those read as state transitions.
 */
...
/** The language id sent to the server in `didOpen`; matches langium-config.json. */
