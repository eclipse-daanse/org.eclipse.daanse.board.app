/**
 * MDX's contribution to the parse tree: the argument-slot strategy.
 *
 * The generic walk is covered in `shared.test.ts`. What is left for MDX is the
 * one hook it installs — rebuilding skippable argument slots from the CST — and
 * a sweep over the bundled examples, which are what the playground loads.
 */
...
import { buildAstTree, type AstTreeNode } from 'org.eclipse.daanse.board.app.lib.lsp.server';
...
describe('argument slots', () => {
    test('a skipped slot becomes an explicit empty row', async () => {
        ...
        // exists to inspect.
        const result = await tree('SELECT ParallelPeriod(, , [Date].[Month]) ON 0 FROM [Sales]');
        const slots = flatten(result.root).filter((row) => row.property === 'arguments');
