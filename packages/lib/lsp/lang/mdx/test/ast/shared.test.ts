/**
 * The language-neutral parse-tree walker.
 *
 * `org.eclipse.daanse.board.app.lib.lsp.core` is shared by every language server in this repo, so its
 * behaviour is tested once here rather than twice in the per-language suites.
 * MDX is used as the vehicle because it is the grammar with every shape the
 * walker handles — nested nodes, multi-valued slots, datatype-rule scalars and
 * cross-references.
 */
import { describe, expect, test } from 'vitest';
import { EmptyFileSystem } from 'langium';
import { parseHelper } from 'langium/test';
import { buildAstTree, renderScalar, type AstTreeNode } from 'org.eclipse.daanse.board.app.lib.lsp.server';
...
/** Every row in the tree, depth-first. */
...
describe('what never reaches the client', () => {
    ...
    test('a reference carries its text, not its target', async () => {
        ...
        const references = flatten(result.root).filter((row) => row.kind === 'reference');
...
describe('source locations', () => {
    ...
    test('every located row falls inside the document', async () => {
        const rows = flatten((await tree(SIMPLE)).root).filter((row) => row.offset !== undefined);
        for (const row of rows) {
    ...
    test('a datatype-rule scalar is still locatable', async () => {
        ...
        const rows = flatten((await tree(SIMPLE)).root);
        const located = rows.filter((row) => row.kind === 'scalar' && row.offset !== undefined);
...
describe('scalar rendering', () => {
    ...
    test('a strategy can override node labels', async () => {
        const document = await parse(SIMPLE);
        const result = buildAstTree(document, { labelNode: (node) => `<${node.$type}>` });
