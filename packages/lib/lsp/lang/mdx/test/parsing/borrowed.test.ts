/**
 * Syntax adopted from `mdx.langium.alt`, the Eclipse Daanse port kept alongside
 * this grammar. Each construct here was a parse or lex error before the merge;
 * `docs/mdx-grammar-comparison.md` records where each one came from and why it
 * was worth taking.
 *
 * These are deliberately end-to-end parse assertions rather than AST-shape
 * assertions — the point of each case is that the text is accepted at all.
 */
...
async function parseExpression(text: string): Promise<Expression> {
    const script = await parseOk(`WITH MEMBER [Measures].[M] AS ${text} SELECT FROM [Cube]`);
...
describe('query parameters', () => {
    ...
    test('parameters work as ordinary operands', async () => {
        await parseOk('SELECT [Date].[Calendar].Members ON 0 FROM [Cube] WHERE @Region');
...
describe('composite member keys', () => {
    test('a two-part key is one member reference', async () => {
        ...
        const access = expression as { key: boolean; segment: { $refText: string }; keys: string[] };
    ...
    test('composite keys are accepted in declaration positions too', async () => {
        await parseOk('WITH MEMBER [Date].[Calendar].&[2005]&[7].[X] AS 1 SELECT FROM [Cube]');
    });
});

describe('CAST', () => {
    test('CAST(expr AS type) is its own node, with a typed target', async () => {
        const expression = await parseExpression("CAST([Measures].[Amount] AS STRING)");
...
describe('AS aliases', () => {
    ...
    test('the alias binds looser than every operator', async () => {
        ...
        expect(((expression as { expression: Expression }).expression as { operator: string }).operator).toBe('+');
    });

    test('WITH MEMBER … AS is still the declaration separator', async () => {
        const script = await parseOk('WITH MEMBER [Measures].[M] AS 1 SET [S] AS {[A]} SELECT FROM [C]');
        const select = script.statements[0] as SelectStatement;
...
describe('skippable arguments', () => {
    test('an omitted slot parses', async () => {
        const expression = await parseExpression('ParallelPeriod(, , [Date].[Calendar].[Month])');
        ...
        expect((expression as { arguments: Expression[] }).arguments).toHaveLength(1);
    });
    ...
    test('an empty argument list is still empty', async () => {
        ...
        expect((expression as { arguments: Expression[] }).arguments).toHaveLength(0);
    });
});
...
describe('statements built around a SELECT', () => {
    test('DRILLTHROUGH with every optional clause', async () => {
        ...
        const drill = statement as { maxRows: number; firstRowSet: number; returns: string[] };
    ...
    test('EXPLAIN PLAN FOR takes either statement', async () => {
        ...
        expect(isSelectStatement((select.statements[0] as { statement: unknown }).statement as never)).toBe(true);

        const drill = await parseOk('EXPLAIN PLAN FOR DRILLTHROUGH SELECT [M] ON 0 FROM [Cube]');
        expect(isDrillthroughStatement((drill.statements[0] as { statement: unknown }).statement as never)).toBe(true);
    });
    ...
    test('REFRESH CUBE', async () => {
        ...
        expect((script.statements[0] as { cube: string }).cube).toBe('[Adventure Works]');
    });
});
...
describe('UPDATE CUBE', () => {
    test('a single writeback with an allocation mode', async () => {
        ...
        const update = statement as { cube: string; clauses: Array<{ allocation?: string }> };
...
describe('FREEZE', () => {
    ...
    test('the parenthesised form still works', async () => {
        ...
        expect((script.statements[0] as { subcube: unknown[] }).subcube).toHaveLength(2);
    });
});
...
describe('soft keywords', () => {
    test('.Dimension is a member property, not the DIMENSION clause', async () => {
        ...
        expect(isMemberAccess(expression)).toBe(true);
        expect((expression as { segment: { $refText: string } }).segment.$refText).toBe('Dimension');
    });
    ...
    test('CUBE stays usable as a bare name', async () => {
        await parseOk('WITH MEMBER [Measures].[X] AS 1, SCOPE_ISOLATION = CUBE SELECT FROM [Cube]');
    });
});
