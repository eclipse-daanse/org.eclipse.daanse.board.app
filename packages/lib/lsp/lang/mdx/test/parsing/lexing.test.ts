...
import type { Expression, SelectStatement } from '../../src/language/generated/ast.js';
...
describe('delimited identifiers', () => {
    test('a bracketed name containing a space parses as one token', async () => {
        const select = await parseSelect('SELECT [Measures].[Sales Volume] ON 0 FROM [Sales Cube]');
    ...
    test(']] escapes a closing bracket', async () => {
        ...
        const select = await parseSelect('SELECT [Total Profit [Domestic]]] ON 0 FROM [Cube]');
    ...
    test('a reserved keyword can be used as a delimited identifier', async () => {
        const select = await parseSelect('SELECT [Measures].[Sales] ON 0 FROM [Cube] WHERE [Product].[select]');
...
describe('string literals', () => {
    test.each([
        ...
        const document = await parseMdx(`WITH MEMBER [Measures].[M] AS ${literal} SELECT FROM [Cube]`);
        expect(syntaxErrors(document)).toBe('');
    });
});

describe('numbers', () => {
    test.each(['1', '1.5', '.5', '1e3', '1.5E-2'])('%s is a numeric literal', async (literal) => {
        const document = await parseMdx(`WITH MEMBER [Measures].[M] AS ${literal} SELECT FROM [Cube]`);
        expect(syntaxErrors(document)).toBe('');
    });
});
...
describe('case insensitivity', () => {
    test('keyword casing does not change the AST', async () => {
        const upper = await parseSelect('SELECT {[A].[B]} ON COLUMNS FROM [Cube] WHERE [C].[D]');
        const lower = await parseSelect('select {[a].[b]} on columns from [Cube] where [c].[d]');
        const mixed = await parseSelect('SeLeCt {[A].[B]} On CoLuMnS FrOm [Cube] WhErE [C].[D]');
