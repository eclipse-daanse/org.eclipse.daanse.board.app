/**
 * The intrinsic-function half of completion. Cube metadata is the playground's
 * concern and is tested in `ui/test/mdx-completion.test.ts`; what the language
 * server contributes is the function table, which is what this covers.
 *
 * The cursor is written as `|` and removed before parsing.
 */
import { describe, expect, test } from 'vitest';
import type { CompletionItem } from 'vscode-languageserver';
import { parseMdx, services } from '../test-helper.js';
import { functionDisplayName, functionNames, lookupFunction } from '../../src/language/mdx-functions.js';
...
describe('expression position', () => {
    ...
    test('arity is shown as the detail', async () => {
        ...
        const detailOf = (label: string) => items.find((i) => i.label === label)?.detail;
    ...
    test('a discouraged function is marked as such', async () => {
        ...
        expect(items.find((i) => i.label === 'LookupCube')?.detail).toContain('discouraged');
    });
});
...
describe('after a dot', () => {
    test('offers the continuations of a member', async () => {
        ...
        expect(labels(items)).toEqual(
            expect.arrayContaining(['Parent', 'CurrentMember', 'Members', 'UniqueName', 'Lag'])
        );
    });
    ...
    test('does not offer calls that cannot follow a member', async () => {
        ...
        for (const name of ['Sum', 'Filter', 'Ytd', 'UserName', 'Root']) {
            expect(labels(items), `${name} is not a postfix continuation`).not.toContain(name);
        }
    });
});
