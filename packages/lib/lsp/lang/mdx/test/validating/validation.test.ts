...
import { diagnostics, diagnosticsOfSeverity, parseMdx, syntaxErrors } from '../test-helper.js';
...
describe('axis ordinals', () => {
    test('an ordinal above 127 is rejected', async () => {
        const document = await validate('SELECT {[A].[B]} ON 0, {[C].[D]} ON 128 FROM [Cube]');
    ...
    test('a duplicated axis is rejected', async () => {
        const document = await validate('SELECT {[A].[B]} ON 0, {[C].[D]} ON COLUMNS FROM [Cube]');
        expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
    ...
    test('a gap in the axis sequence is rejected', async () => {
        const document = await validate('SELECT {[A].[B]} ON 0, {[C].[D]} ON 2 FROM [Cube]');
        expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
    ...
    test('COLUMNS then ROWS is accepted', async () => {
        const document = await validate('SELECT {[A].[B]} ON COLUMNS, {[C].[D]} ON ROWS FROM [Cube]');
        expect(diagnosticsOfSeverity(document, ERROR)).toHaveLength(0);
    });

    test('AXIS(0) and AXIS(1) are accepted', async () => {
        const document = await validate('SELECT {[A].[B]} ON AXIS(0), {[C].[D]} ON AXIS(1) FROM [Cube]');
        expect(diagnosticsOfSeverity(document, ERROR)).toHaveLength(0);
    });
});
...
describe('calculated member properties', () => {
    test('an unknown property warns', async () => {
        ...
        expect(diagnosticsOfSeverity(document, WARNING)).toContainEqual(
            expect.stringContaining("Unknown calculated member property 'NOT_A_PROPERTY'")
        );
    });
...
describe('function calls', () => {
    test('an unknown function warns', async () => {
        const document = await validate('WITH MEMBER [Measures].[X] AS NotAFunction(1) SELECT FROM [Cube]');
        expect(diagnosticsOfSeverity(document, WARNING)).toContainEqual(
    ...
    test('too few arguments is an error', async () => {
        const document = await validate('WITH MEMBER [Measures].[X] AS IIF(1) SELECT FROM [Cube]');
        expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
            expect.stringContaining("'IIF' expects at least 3 argument(s), but got 1")
        );
    });
    ...
    describe('skipped argument slots still count', () => {
        ...
        test('a genuinely short call is still an error', async () => {
            ...
            expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
                expect.stringContaining("'IIF' expects at least 3 argument(s), but got 2")
            );
        });
        ...
        test('an empty list is zero slots, not one', async () => {
            ...
            expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
                expect.stringContaining("'IIF' expects at least 3 argument(s), but got 0")
            );
        });
        ...
        test('too many slots is still an error', async () => {
            ...
            expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
                expect.stringContaining("'IIF' expects at most 3 argument(s), but got 5")
            );
        });
    });
    ...
    test('too many arguments is an error', async () => {
        ...
        expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
            expect.stringContaining("'KPIValue' expects at most 1 argument(s), but got 2")
        );
    });
...
describe('cell properties', () => {
    test('an unknown cell property warns', async () => {
        const document = await validate('SELECT FROM [Cube] CELL PROPERTIES NOT_A_PROPERTY');
        expect(diagnosticsOfSeverity(document, WARNING)).toContainEqual(
...
describe('identifiers', () => {
    test('a reserved keyword used undelimited warns', async () => {
        ...
        const document = await validate('WITH SET Absolute AS {[A].[B]} SELECT FROM [Cube]');
        expect(diagnosticsOfSeverity(document, WARNING)).toContainEqual(
    ...
    test('a keyword that the grammar itself claims cannot be used undelimited', async () => {
        ...
        const document = await parseMdx('WITH SET Order AS {[A].[B]} SELECT FROM [Cube]');
        expect(document.parseResult.parserErrors.length).toBeGreaterThan(0);

        const bracketed = await validate('WITH SET [Order] AS {[A].[B]} SELECT FROM [Cube]');
        expect(diagnostics(bracketed)).toHaveLength(0);
    });

    test('an identifier over 100 characters is rejected', async () => {
        const long = 'A'.repeat(101);
        const document = await validate(`WITH SET [${long}] AS {[A].[B]} SELECT FROM [Cube]`);
        expect(diagnosticsOfSeverity(document, ERROR)).toContainEqual(
    ...
    test('a delimited reserved keyword is accepted', async () => {
        const document = await validate('WITH SET [Order] AS {[A].[B]} SELECT FROM [Cube]');
        expect(diagnostics(document)).toHaveLength(0);
    });
});
...
describe('CREATE MEMBER cube target', () => {
    test('naming a cube other than CURRENTCUBE warns', async () => {
        const document = await validate('CREATE MEMBER [Other Cube].[Measures].[X] AS 1');
        expect(diagnosticsOfSeverity(document, WARNING)).toContainEqual(
    ...
    test('CURRENTCUBE is accepted', async () => {
        const document = await validate('CREATE MEMBER CURRENTCUBE.[Measures].[X] AS 1');
        expect(diagnostics(document)).toHaveLength(0);
    });
...
describe('slicer axis', () => {
    test('a multi-member set in WHERE warns', async () => {
        const document = await validate('SELECT {[A].[B]} ON 0 FROM [Cube] WHERE {[C].[D], [C].[E]}');
        expect(diagnosticsOfSeverity(document, WARNING)).toContainEqual(
    ...
    test('a tuple in WHERE is accepted', async () => {
        const document = await validate('SELECT {[A].[B]} ON 0 FROM [Cube] WHERE ([C].[D], [E].[F])');
        expect(diagnostics(document)).toHaveLength(0);
    });
});
...
describe('the bundled examples validate cleanly', () => {
    test.each([
        ...
        const text = readFileSync(join(__dirname, '..', '..', 'examples', file), 'utf8');
        const document = await parseMdx(text, { validation: true });
        expect(syntaxErrors(document)).toBe('');
        expect(diagnosticsOfSeverity(document, ERROR)).toHaveLength(0);
    });
