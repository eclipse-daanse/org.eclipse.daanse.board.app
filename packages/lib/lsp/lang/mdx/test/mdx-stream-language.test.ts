...
describe('delimited identifiers', () => {
  test('a bracketed name is one token, even when it contains a keyword', () => {
    ...
    expect(tokens.find(t => t.text === '[Order Quantity]')?.kind).toBe(
      'variableName',
    )
  ...
  test('a doubled ]] is an escape, not a terminator', () => {
    ...
    expect(tokens.find(t => t.kind === 'variableName')?.text).toBe(
      '[Total Profit [Domestic]]]',
    )
  })

  test('a key qualifier is its own category', () => {
    expect(kindOf('[Product].[Model].&[Mountain-200]', '&[Mountain-200]')).toBe(
      'atom',
    )
  })
})
...
describe('keywords and operators', () => {
  ...
  test.each(['AND', 'OR', 'NOT', 'XOR', 'IS'])(
    '%s is a word operator',
    word => {
...
describe('comments', () => {
  ...
  test('-- is a comment, not two minus signs', () => {
    expect(
      tokenize('-- Source: SELECT Statement').every(t => t.kind === 'comment'),
    ).toBe(true)
  })
...
describe('literals and calls', () => {
  ...
  test('a name after a dot is a property', () => {
    expect(kindOf('[Date].[Calendar].CurrentMember', 'CurrentMember')).toBe(
      'propertyName',
    )
  })

  test('a member property is only highlighted where it is assigned', () => {
    expect(kindOf('MEMBER x AS 1, SOLVE_ORDER = 5', 'SOLVE_ORDER')).toBe(
      'propertyName',
    )
