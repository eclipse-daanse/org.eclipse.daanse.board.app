...
/**
 * Classifies the text as if the cursor sat at its end — the only position the
 * completion provider ever asks about. Columns are 1-based, matching Monaco.
 */
function at(text: string) {
    return parseMdxCompletionContext(text, text.length + 1);
}

/** The substring the parser says a suggestion would replace. */
function replaced(text: string) {
    const ctx = at(text);
    if (ctx.kind === 'none') throw new Error('no word range for a suppressed context');
...
describe('bracket chains', () => {
    ...
    test('two closed segments offer levels', () => {
        ...
        if (ctx.kind !== 'bracket-chain') throw new Error('expected a bracket chain');
...
/**
 * `chainRange` covers the whole chain, not just the segment under the caret.
 * Suggestions carrying a fully-qualified unique name replace that instead of
 * `wordRange`, or the segments to their left survive in front of them.
 */
describe('chainRange', () => {
  ...
  test('covers a two-segment chain', () => {
    ...
    expect(ctx).toMatchObject({ kind: 'bracket-chain', chainRange: { start: 8, end: 30 } })
  })

  test('covers a chain whose caret sits right after a closed segment', () => {
    const ctx = parseMdxCompletionContext('SELECT [Product]', 17)
    expect(ctx).toMatchObject({ kind: 'bracket-chain', chainRange: { start: 8, end: 17 } })
  })
