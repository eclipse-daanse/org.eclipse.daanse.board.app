...
function If() {
  ...
  o.prototype.swap16 = function() {
    ...
    let F = N - C, k = S - l, B = Math.min(F, k), re2 = this.slice(C, N), ie = u.slice(l, S);
    for (let oe = 0; oe < B; ++oe) if (re2[oe] !== ie[oe]) {
      F = re2[oe], k = ie[oe];
      break;
    }
  ...
  function O(p, u, l, S, C) {
    ...
    if (C) {
      let ie = -1;
      for (re2 = l; re2 < F; re2++) if (B(p, re2) === B(u, ie === -1 ? 0 : re2 - ie)) {
        if (ie === -1 && (ie = re2), re2 - ie + 1 === k) return ie * N;
      } else ie !== -1 && (re2 -= re2 - ie), ie = -1;
    } else for (l + k > F && (l = F - k), re2 = l; re2 >= 0; re2--) {
      let ie = true;
      for (let oe = 0; oe < k; oe++) if (B(p, re2 + oe) !== B(u, oe)) {
        ie = false;
        break;
      }
      if (ie) return re2;
      ...
      if (C + k <= l) {
        ...
        switch (k) {
          ...
          case 2:
            ...
            break;
          case 3:
            B = p[C + 1], re2 = p[C + 2], (B & 192) === 128 && (re2 & 192) === 128 && (oe = (N & 15) << 12 | (B & 63) << 6 | re2 & 63, oe > 2047 && (oe < 55296 || oe > 57343) && (F = oe));
            break;
          case 4:
            B = p[C + 1], re2 = p[C + 2], ie = p[C + 3], (B & 192) === 128 && (re2 & 192) === 128 && (ie & 192) === 128 && (oe = (N & 15) << 18 | (B & 63) << 12 | (re2 & 63) << 6 | ie & 63, oe > 65535 && oe < 1114112 && (F = oe));
        }
      }
...
var zh = D((Gh) => {
  ...
  function C_(t) {
    var e = this.lastTotal - this.lastNeed, r = k_(this, t);
...
var Ba = D((cN, ey) => {
  ...
  function aE(t, e, r) {
    ...
    a === void 0 && (a = true);
    let y = 0;
    if (typeof i != "string" && !se.isBuffer(i)) return e.destroy(new Error("Invalid protocolId")), false;
...
var vy = D((_o) => {
  ...
  var NE = (t) => !wy.has(t), lr = (t) => t && t === Math.floor(t) && t > 0 && isFinite(t), _y = (t) => lr(t) ? t <= Math.pow(2, 8) ? Uint8Array : t <= Math.pow(2, 16) ? Uint16Array : t <= Math.pow(2, 32) ? Uint32Array : t <= Number.MAX_SAFE_INTEGER ? bn : null : null, bn = class extends Array {
    constructor(e) {
      super(e), this.fill(0);
    }
  }, $a = class t {
...
function Px() {
  ...
  function V(k, B) {
    ...
    var re2 = /[0-9](?=(?:[0-9]{3})+(?![0-9]))/g;
    if (typeof k == "number") {
      var ie = k < 0 ? -j(-k) : j(k);
      if (ie !== k) {
        var oe = String(ie), ce = v.call(B, oe.length + 1);
        return R.call(oe, re2, "$&_") + "." + R.call(R.call(ce, /([0-9]{3})/g, "$&_"), /_$/, "");
      }
    }
    return R.call(B, re2, "$&_");
  }
  var te = Tx, Y = te.custom, ee = Oe(Y) ? Y : null;
  El = function k(B, re2, ie, oe) {
    var ce = re2 || {};
  ...
  function pe(k, B, re2) {
    var ie = (re2.quoteStyle || B) === "double" ? '"' : "'";
    return ie + k + ie;
  }
  function Z(k) {
  ...
  function Bt(k, B) {
    ...
    for (var re2 = 0, ie = k.length; re2 < ie; re2++) if (k[re2] === B) return re2;
    return -1;
  }
  function xn(k) {
  ...
  function at(k, B) {
    if (k.length > B.maxStringLength) {
      var re2 = k.length - B.maxStringLength, ie = "... " + re2 + " more character" + (re2 > 1 ? "s" : "");
  ...
  function Zo(k) {
    var B = k.charCodeAt(0), re2 = { 8: "b", 9: "t", 10: "n", 12: "f", 13: "r" }[B];
    return re2 ? "\\" + re2 : "\\x" + (B < 16 ? "0" : "") + T.call(B.toString(16));
  }
  function p(k) {
    return "Object(" + k + ")";
  }
  ...
  function l(k, B, re2, ie) {
    var oe = ie ? N(re2, ie) : q.call(re2, ", ");
    return k + " (" + B + ") {" + oe + "}";
  }
  function S(k) {
...
`) >= 0) return false;
  ...
  function C(k, B) {
    var re2;
    if (k.indent === "	") re2 = "	";
    else if (typeof k.indent == "number" && k.indent > 0) re2 = q.call(Array(k.indent + 1), " ");
    else return null;
    return { base: re2, prev: q.call(Array(B + 1), re2) };
  }
  function N(k, B) {
    if (k.length === 0) return "";
    var re2 = `
` + B.prev + B.base;
    return re2 + q.call(k, "," + re2) + `
` + B.prev;
  }
  function F(k, B) {
    var re2 = ne(k), ie = [];
    ...
    for (var Ye in k) Re(k, Ye) && (re2 && String(Number(Ye)) === Ye && Ye < k.length || G && Qt["$" + Ye] instanceof Symbol || (O.call(/[^\w$]/, Ye) ? ie.push(B(Ye, k) + ": " + B(k[Ye], k)) : ie.push(Ye + ": " + B(k[Ye], k))));
...
let MQTTConnection = class extends TwoWayConnection {
  loggerFactory;
  logMqtt;
  url;
  client = null;
...
], MQTTConnection.prototype, "loggerFactory", 2);
MQTTConnection = __decorateClass([
  injectable()
], MQTTConnection);
const MQTT_CONNECTION_FACTORY = "MQTTConnectionFactory";
const factorySymbol = Symbol.for(MQTT_CONNECTION_FACTORY);
function activate$1({ services }) {
  services.register(MQTT_CONNECTION_FACTORY, (config) => {
...
const LIBRARY_ID = "org.eclipse.daanse.board.app.lib.connection.mqtt";
...
async function activate(context) {
  ...
  runtime.register(LIBRARY_ID, library, VERSION, "lib.connection.mqtt");
