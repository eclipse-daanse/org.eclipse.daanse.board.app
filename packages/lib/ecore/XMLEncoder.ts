...
export class XMLEncoder implements EEncoder {
                    ...
                    case SaveFeatureKind.ObjectHREFSingle: {
                        switch (this.getSaveResourceKindSingle(eObject, eFeature)) {
                            case SaveResourceKind.Cross: {
                                ...
                                this.saveIDRefCross(eObject, eFeature)
                                continue LOOP
                            }
                            case SaveResourceKind.Same: {
                                this.saveIDRefSingle(eObject, eFeature)
                    ...
                    case SaveFeatureKind.ObjectHREFMany: {
                        switch (this.getSaveResourceKindMany(eObject, eFeature)) {
                            case SaveResourceKind.Cross: {
                                ...
                                this.saveIDRefManyCross(eObject, eFeature)
  ...
  private saveIDRefManyCross(eObject: EObject, eFeature: EStructuralFeature) {
    let l = eObject.eGetResolve(eFeature, false) as EList<EObject>
    let failure = false
    let buffer = ""
    let first = true
    for (const value of l) {
      if (value) {
        let id = this.getIDCrossRef(value)
        if (id == "") {
          failure = true
        } else {
          if (!first) {
  ...
  private saveIDRefCross(eObject: EObject, eFeature: EStructuralFeature) {
    let value = eObject.eGetResolve(eFeature, false) as EObject
    let failure = false
    let buffer = ""
    let first = true
      if (value) {
        let id = /*EcoreUtils.getURI(value)+'';*/this.getIDCrossRef(value)
        if (id == "") {
          failure = true
        } else {
          if (!first) {
  ...
  private getIDCrossRef(eObject: EObject): string {
    //const  result:EObject = eObject.eGet(eFeature);
    const epn = eObject.eClass().ePackage.name;
    const ecn = eObject.eClass().name;
    const name = epn + ":" + ecn + " " + EcoreUtils.getURI(eObject);
    return name
  }
