...
export class XMLDecoder implements EDecoder {
    ...
    private handleFeature(uri: string, local: string) {
        ...
        if (eObject) {
            // Special handling for eTypeParameters on EClassifier
            if (local === "eTypeParameters") {
                // Create ETypeParameter and add to eTypeParameters list
                let eFactory = getEcorePackage().eFactoryInstance
                let typeParameter = eFactory.createETypeParameter()

                // Handle attributes (like name)
                this.handleAttributes(typeParameter)

                // Add to classifier's eTypeParameters
                if ((eObject as any).eTypeParameters) {
                    (eObject as any).eTypeParameters.add(typeParameter)
                }

                // Push for processing child elements
                this.processObject(typeParameter)
                return
            }

            // Special handling for eGenericType XML elements
            if (local === "eGenericType" && this.isETypedElement(eObject)) {
                console.log("DEBUG: Found eGenericType element for ETypedElement:", eObject.eClass().name)
                this.handleEGenericType(eObject)
                return
            }

            // Special handling for eGenericType child elements
            if (this.isEGenericType(eObject)) {
                console.log("DEBUG: Processing child element", local, "for eGenericType")
                if (this.handleEGenericTypeChildren(eObject, local)) {
                    return
                }
            }
    ...
    private handleReferences() {
        ...
        for (const reference of this._references) {
            console.log("DEBUG: Processing reference:", reference.id, "for object:", reference.object.eClass().name, "feature:", reference.feature.name)
            let eObject = this._resource.getEObject(reference.id)
            console.log("DEBUG: getEObject result:", eObject ? "FOUND" : "NULL")
            // If not found in current resource, try to resolve external URI
            if (!eObject && (reference.id.includes(":") || reference.id.includes("http://"))) {
                console.log("DEBUG: Attempting external URI resolution for:", reference.id)
                eObject = this.resolveExternalReference(reference.id)
                console.log("DEBUG: External resolution result:", eObject ? "FOUND" : "NULL")
            }

            if (eObject) {
                this.setFeatureValue(reference.object, reference.feature, eObject, reference.pos)
                console.log("DEBUG: Successfully set reference")
            } else {
                console.log("DEBUG: Failed to resolve reference '" + reference.id + "'")
                // Create a proxy object for unresolved references
                let proxyObject = new BasicEObject()
                let proxyURI = new URI(reference.id)
                proxyObject.eSetProxyURI(proxyURI)
                this.setFeatureValue(reference.object, reference.feature, proxyObject, reference.pos)
                console.log("DEBUG: Created proxy for unresolved reference")

                // For cross-document references, try to create a proxy or handle specially
                if (reference.id.includes("http://")) {
                    console.log("DEBUG: Cross-document reference detected, skipping error")
                    // TODO: Handle cross-document references properly
                } else {
                    this.error(
                        new EDiagnosticImpl(
    ...
    private resolveExternalReference(id: string): EObject {
        // Handle external references like "ecore:EDataType http://www.eclipse.org/emf/2002/Ecore#//EString"
        // Also handle direct URI references like "org.eclipse.daanse.board.app.ui.vue.composables#//ValueWrapper"
        let prefix = ""
        let uriFragment = ""

        // Parse format: "prefix:EDataType URI#//fragment" or "URI#//fragment"
        let colonIndex = id.indexOf(":")
        if (colonIndex != -1) {
            prefix = id.slice(0, colonIndex)
            let remainder = id.slice(colonIndex + 1)

            // Find the URI part and fragment part
            let fragmentIndex = remainder.indexOf("#//")
            if (fragmentIndex != -1) {
                let uri = remainder.slice(remainder.indexOf(" ") + 1, fragmentIndex)
                uriFragment = remainder.slice(fragmentIndex + 3) // Skip "#//"

                console.log("DEBUG: Parsed external reference - prefix:", prefix, "uri:", uri, "fragment:", uriFragment)

                // Get factory for the URI
                let eFactory = this.getFactoryForURI(uri)
                if (eFactory) {
                    let ePackage = eFactory.ePackage

                    // Try to find the classifier by name in the package
                    if (uriFragment) {
                        for (let classifier of ePackage.eClassifiers) {
                            if (classifier.name === uriFragment) {
                                console.log("DEBUG: Found external classifier:", classifier.name, "in package:", ePackage.name)
                                return classifier
                            }
                        }
                    }

                    console.log("DEBUG: Could not find classifier '" + uriFragment + "' in package '" + ePackage.name + "'")
                } else {
                    console.log("DEBUG: Could not find factory for URI:", uri)
                }
            } else {
                console.log("DEBUG: Could not parse fragment from external reference:", id)
            }
        } else {
            // Handle direct URI format: "packageURI#//ClassName"
            let fragmentIndex = id.indexOf("#//")
            if (fragmentIndex != -1) {
                let uri = id.slice(0, fragmentIndex)
                uriFragment = id.slice(fragmentIndex + 3) // Skip "#//"

                console.log("DEBUG: Parsed direct URI reference - uri:", uri, "fragment:", uriFragment)

                // For unresolvable external packages, we'll create a proxy placeholder
                // This allows the template to know about the reference even if we can't resolve it
                console.log("DEBUG: Creating proxy for unresolvable external reference")
                return null // Return null but the reference info is preserved
            } else {
                console.log("DEBUG: Could not parse direct URI reference:", id)
            }
        }
    ...
    private isETypedElement(eObject: EObject): boolean {
        // Check if the object is an instance of ETypedElement (or subclass)
        let eClass = eObject.eClass()
        let eTypedElementClass = getEcorePackage().getETypedElement()

        // Direct match
        if (eClass === eTypedElementClass) {
            return true
        }

        // Check supertype hierarchy
        let allSuperTypes = eClass.eAllSuperTypes
        if (allSuperTypes) {
            for (let i = 0; i < allSuperTypes.size(); i++) {
                if (allSuperTypes.get(i) === eTypedElementClass) {
                    return true
                }
            }
        }
    ...
    private handleEGenericType(eTypedElement: EObject) {
        // Create an EGenericType instance
        let eFactory = getEcorePackage().eFactoryInstance
        let eGenericType = eFactory.createEGenericType() as EGenericType

        // Handle eClassifier attribute on the eGenericType element itself
        let eClassifierValue = this.getAttributeValue("", "eClassifier")
        if (eClassifierValue) {
            let id = eClassifierValue.startsWith("#") ? eClassifierValue.substring(1) : eClassifierValue
            console.log("DEBUG: Adding main eGenericType.eClassifier reference:", id)
            this._references.push({
                object: eGenericType,
                feature: getEcorePackage().getEGenericType_EClassifier(),
                id: id,
                pos: -1
            })
        }

        // Set it as the eGenericType property of the ETypedElement
        if (this.isETypedElement(eTypedElement)) {
            (eTypedElement as any).eGenericType = eGenericType
        }

        // Push the EGenericType to the objects stack for processing its children
        this._types.push("eGenericType")
        this._objects.push(eGenericType)
    }

    private isEGenericType(eObject: EObject): boolean {
        // Check if the object is an instance of EGenericType
        let eClass = eObject.eClass()
        let eGenericTypeClass = getEcorePackage().getEGenericType()
        return eClass === eGenericTypeClass
    }

    private handleEGenericTypeChildren(eGenericType: EObject, local: string): boolean {
        // Handle child elements of eGenericType: eClassifier, eTypeArguments, etc.
        switch (local) {
            case "eTypeArguments": {
                console.log("DEBUG: Processing eTypeArguments for eGenericType")
                // Create a new EGenericType for the type argument
                let eFactory = getEcorePackage().eFactoryInstance
                let typeArgGenericType = eFactory.createEGenericType() as EGenericType

                // Check if there's an eClassifier attribute (for direct type reference)
                let eClassifierValue = this.getAttributeValue("", "eClassifier")
                if (eClassifierValue) {
                    // Handle eClassifier attribute by adding to references for later resolution
                    // For external references like "ecore:EDataType ...", handle differently
                    console.log("DEBUG: Adding eTypeArguments eClassifier reference:", eClassifierValue)
                    this._references.push({
                        object: typeArgGenericType,
                        feature: getEcorePackage().getEGenericType_EClassifier(),
                        id: eClassifierValue,
                        pos: -1
                    })
                ...
                // Add the type argument to the parent's eTypeArguments collection
                let eTypeArguments = (eGenericType as any).eTypeArguments
                if (eTypeArguments) {
                    console.log("DEBUG: Adding typeArgGenericType to parent's eTypeArguments collection")
                    eTypeArguments.add(typeArgGenericType)
                } else {
                    console.log("DEBUG: WARNING - parent eGenericType does not have eTypeArguments collection")
                }

                // Push to stack for processing any child elements
                this._types.push("eGenericType")
                this._objects.push(typeArgGenericType)
                return true
            }
            case "eClassifier": {
                // Handle eClassifier reference from href attribute
                let hrefValue = this.getAttributeValue("", "href")
                if (hrefValue) {
                    // Handle href attribute by adding to references for later resolution
                    // For same-document references, remove the # prefix
                    let id = hrefValue.startsWith("#") ? hrefValue.substring(1) : hrefValue
                    console.log("DEBUG: Adding eClassifier reference:", id)
                    this._references.push({
                        object: eGenericType,
                        feature: getEcorePackage().getEGenericType_EClassifier(),
                        id: id,
                ...
                // Don't push to stack since this is just setting a reference
                this._types.push("attribute")
                this._objects.push(null)
                return true
            }
            case "eTypeParameter":
            case "eUpperBound":
            case "eLowerBound": {
                // These are single-valued references - handle as regular feature
                let eFeature = this.getFeature(eGenericType, local)
                if (eFeature) {
                    let xsiType = this.getXSIType()
                    if (xsiType) {
                        this.createObjectFromTypeName(eGenericType, xsiType, eFeature)
                    } else {
                        this.createObjectFromFeatureType(eGenericType, eFeature)
                    }
                    return true
                }
                break
