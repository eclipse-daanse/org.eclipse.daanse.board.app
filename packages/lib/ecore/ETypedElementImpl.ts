...
export class ETypedElementImpl extends ENamedElementImpl implements ETypedElement {
    ...
    protected _eGenericType: EGenericType
    ...
    set eGenericType(newEGenericType: EGenericType) {
        let oldEGenericType = this._eGenericType
        if (newEGenericType != oldEGenericType) {
            let notifications: ENotificationChain = null
            if (isEObjectInternal(oldEGenericType)) {
                notifications = oldEGenericType.eInverseRemove(
                    this,
                    EOPPOSITE_FEATURE_BASE - EcoreConstants.ETYPED_ELEMENT__EGENERIC_TYPE,
                    notifications
                )
            }
            if (isEObjectInternal(newEGenericType)) {
                notifications = newEGenericType.eInverseAdd(
                    this,
                    EOPPOSITE_FEATURE_BASE - EcoreConstants.ETYPED_ELEMENT__EGENERIC_TYPE,
                    notifications
                )
            }
            notifications = this.basicSetEGenericType(newEGenericType, notifications)
    ...
    basicSetEGenericType(newEGenericType: EGenericType, msgs: ENotificationChain): ENotificationChain {
        let oldEGenericType = this._eGenericType
        this._eGenericType = newEGenericType
        let notifications = msgs
        if (this.eNotificationRequired) {
            let notification = new Notification(
                this,
                EventType.SET,
                EcoreConstants.ETYPED_ELEMENT__EGENERIC_TYPE,
                oldEGenericType,
                newEGenericType
            )
            if (notifications != null) {
    ...
    eGetFromID(featureID: number, resolve: boolean): any {
        switch (featureID) {
            ...
            case EcoreConstants.ETYPED_ELEMENT__EGENERIC_TYPE: {
    ...
    eSetFromID(featureID: number, newValue: any) {
        switch (featureID) {
            ...
            case EcoreConstants.ETYPED_ELEMENT__EGENERIC_TYPE: {
                this.eGenericType = newValue as EGenericType
                break
            }
            case EcoreConstants.ETYPED_ELEMENT__LOWER_BOUND: {
    ...
    eBasicInverseRemove(otherEnd: any, featureID: number, notifications: ENotificationChain): ENotificationChain {
        // eGenericType is not a registered structural feature, so no need to handle it here
