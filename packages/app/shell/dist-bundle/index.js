...
import { ref as ee, defineComponent as Qe, computed as ne, inject as xe, onMounted as Hn, onBeforeUnmount as Qo, createElementBlock as A, openBlock as _, Fragment as Ce, createElementVNode as u, createBlock as Be, createCommentVNode as re, renderList as Ge, unref as b, toDisplayString as z, normalizeClass as Ee, withModifiers as Mn, createStaticVNode as l_, Teleport as Gl, resolveComponent as u_, createVNode as B, shallowRef as Bl, watch as Nn, nextTick as gi, normalizeStyle as Yt, resolveDynamicComponent as Kn, createTextVNode as ae, withDirectives as Kt, vModelText as Ho, withCtx as te, withKeys as vi, mergeModels as hi, useModel as jo, renderSlot as d_, vShow as zo, Transition as c_, isRef as f_, createApp as p_ } from "vue";
...
import { useEList as Tn, useGlobalLoading as h_, useEObject as Yl, useFeature as __, useBoard as m_, VariableComplexStringWrapper as b_, VARIABLECOMPLEXSTRINGWRAPPER as y_ } from "org.eclipse.daanse.board.app.ui.vue.composables";
import { DIcon as Xe, DButton as be, DInput as gn, DModal as Ko, DSelect as Un, DTabs as w_, DColorInput as Zl, DSwitch as Jl, DFloatingWindow as k_, DDateInput as x_, DSlider as S_, DCheckbox as C_, DChip as qt, DDivider as A_ } from "org.eclipse.daanse.board.app.ui.vue.controls";
import { useRoute as br, useRouter as ea, createRouter as I_, createWebHistory as $_ } from "vue-router";
...
import { identifier as at, WorkspaceImpl as R_, ConnectionImpl as E_, DatasourceImpl as T_, VariableImpl as O_, EventMappingImpl as Xl, PageImpl as L_, WidgetImpl as B_, LayoutItemImpl as D_ } from "org.eclipse.daanse.board.app.lib.model.workspace";
import { JSONResource as Ql, URI as jl, OPTION_INDENT as W_ } from "@emfts/core";
import { identifier as na, CONNECTION_REPOSITORY as V_ } from "org.eclipse.daanse.board.app.lib.api.connection";
import { identifier as ta, DATASOURCE_REPOSITORY as P_ } from "org.eclipse.daanse.board.app.lib.api.datasource";
import { identifier as eu } from "org.eclipse.daanse.board.app.lib.api.variable";
import { identifier as M_ } from "org.eclipse.daanse.board.app.lib.api.persistence";
import { identifier as nu } from "org.eclipse.daanse.board.app.lib.api.widget";
import U_ from "vuedraggable";
import { WrapperSettingsPackage as N_, wrapperSettingsFormXmi as F_, WrapperSettingsImpl as z_, WidgetWrapper as q_ } from "org.eclipse.daanse.board.app.ui.vue.widget.wrapper";
import { SettingsForm as _i, isModelled as G_ } from "org.eclipse.daanse.board.app.ui.vue.uimodel";
import { VariableEvents as Wl } from "org.eclipse.daanse.board.app.lib.variables";
import { SystemActionsEcoreContent as H_, WidgetAction as K_ } from "org.eclipse.daanse.board.app.lib.events";
...
const Z_ = new Uint8Array(16);
function J_() {
  return crypto.getRandomValues(Z_);
}
function ru(g, h, o) {
  return crypto.randomUUID ? crypto.randomUUID() : X_(g);
}
function X_(g, h, o) {
  g = g || {};
  const y = g.random ?? g.rng?.() ?? J_();
  if (y.length < 16)
    throw new Error("Random bytes length must be >= 16");
  return y[6] = y[6] & 15 | 64, y[8] = y[8] & 63 | 128, Y_(y);
}
const qo = ee(void 0);
function iu() {
  return {
...
const Q_ = "data-board-backdrop", au = "daanse.board.backdrop";
function j_() {
  try {
...
const _r = ee(j_());
function su() {
  document.documentElement.setAttribute(Q_, _r.value ? "on" : "off");
}
function em() {
  try {
...
const rm = "data-board-snap", lu = "daanse.board.snap";
function im() {
  try {
...
const mr = ee(im());
function uu() {
  document.documentElement.setAttribute(rm, mr.value ? "on" : "off");
}
function om() {
  try {
...
const lm = { class: "topbar" }, um = {
  ...
  class: "crumb-sep",
  "aria-hidden": "true"
}, cm = ["onClick"], fm = ["aria-expanded"], pm = { class: "pages__name" }, gm = {
  key: 0,
  class: "pages__menu",
  role: "menu"
}, vm = ["onClick"], hm = ["title", "aria-label", "onClick"], _m = ["aria-pressed"], mm = ["aria-pressed"], bm = ["aria-pressed"], ym = {
  key: 6,
  class: "modes",
  role: "group",
  "aria-label": "Modus"
}, wm = ["aria-pressed"], km = ["aria-pressed"], xm = /* @__PURE__ */ Qe({
  __name: "Header",
  setup(g) {
    const h = br(), o = ea(), y = ne(() => h.params.pageid ?? ""), m = ne(
      () => h.name === "edit" || h.name === "pageEdit" || String(h.path).endsWith("/edit")
    ), I = ne(() => {
      const F = { label: "Boards", to: "/" }, N = {
        label: y.value ? `Board ${y.value.slice(0, 8)}` : "Board",
        to: y.value ? `/page/${y.value}` : void 0
      };
      switch (h.name) {
        case "home":
          return [{ label: "Boards" }];
    ...
    }), c = ne(() => !!y.value), E = xe(yr);
    xe(yi);
    const { openSettings: C } = iu(), f = Tn(xe(at), (F) => F.pages), { visible: w, toggle: K } = ou(), { shown: q, toggle: G } = nm(), { snapping: O, toggle: Y } = am(), V = ne(() => {
      if (f.value, !y.value) return !1;
      try {
        const F = E?.getPage(y.value);
      ...
      f.value;
      const F = E?.getAllPageIds() ?? [], N = [];
      for (const M of F) {
        const pe = E?.getPage(M);
        pe && N.push(pe);
      }
      return N;
    }), D = ne(() => {
      if (f.value, !y.value) return "";
      try {
        return E?.getPage(y.value)?.name ?? "Seite";
      } catch {
        return "Seite";
      }
    }), J = ee(!1);
    function ue(F) {
      J.value = !1, F !== y.value && o.push(m.value ? `/page/${F}/edit` : `/page/${F}`);
    }
    const P = ee();
    function Q(F) {
      J.value && (P.value?.contains(F.target) || (J.value = !1));
    }
    function ve(F) {
      F.key === "Escape" && (J.value = !1);
    }
    Hn(() => {
      document.addEventListener("pointerdown", Q), document.addEventListener("keydown", ve);
    }), Qo(() => {
      document.removeEventListener("pointerdown", Q), document.removeEventListener("keydown", ve);
    });
    function Ve() {
      const F = ru();
      E?.registerPage({
      ...
      }), J.value = !1, o.push(`/page/${F}/edit`);
    }
    const ie = ne(() => $.value.length > 1);
    function ce(F) {
      if (!ie.value) return;
      const N = $.value.find((M) => M.id === F);
      if (confirm(`Seite „${N?.name ?? F}" löschen? Das lässt sich nicht rückgängig machen.`) && (E?.unregisterPage(F), F === y.value)) {
        const M = $.value.find((pe) => pe.id !== F);
        M && o.push(m.value ? `/page/${M.id}/edit` : `/page/${M.id}`);
      }
    }
    const de = () => {
      y.value && o.push(`/page/${y.value}`);
    }, $e = () => {
      y.value && o.push(`/page/${y.value}/edit`);
      ...
      u("header", lm, [
        ...
        u("nav", um, [
          (_(!0), A(Ce, null, Ge(I.value, (M, pe) => (_(), A(Ce, {
            ...
            pe > 0 ? (_(), A("span", dm, "/")) : re("", !0),
            M.to && pe < I.value.length - 1 ? (_(), A("button", {
              key: 1,
              type: "button",
              class: "crumb-part crumb-link",
              onClick: (sn) => b(o).push(M.to)
            }, z(M.label), 9, cm)) : (_(), A("span", {
              key: 2,
        ...
        c.value ? (_(), A("div", {
          ...
          u("button", {
            ...
            onClick: N[0] || (N[0] = (M) => J.value = !J.value)
          }, [
            u("span", pm, z(D.value), 1),
            N[5] || (N[5] = u("span", {
              class: "pages__caret",
              "aria-hidden": "true"
            }, "▾", -1))
          ...
          J.value ? (_(), A("ul", gm, [
            (_(!0), A(Ce, null, Ge($.value, (M) => (_(), A("li", {
              key: M.id,
              role: "none",
              class: "pages__line"
            }, [
              u("button", {
                type: "button",
                role: "menuitem",
                class: Ee(["pages__item", { on: M.id === y.value }]),
                onClick: (pe) => ue(M.id)
              }, z(M.name), 11, vm),
              ie.value ? (_(), A("button", {
                key: 0,
        ...
        m.value && V.value ? (_(), A("button", {
          key: 2,
          type: "button",
          class: Ee(["icon-action", { on: b(q) }]),
          "aria-pressed": b(q),
  ...
  for (const [y, m] of h)
    o[y] = m;
  return o;
}, Sm = /* @__PURE__ */ je(xm, [["__scopeId", "data-v-b0469dc0"]]), mi = [
  {
    id: "surfaces",
    label: "Flächen",
    ...
    return {
      ...
      overrides: Object.fromEntries(
        Object.entries(o?.overrides ?? {}).filter(
          ([y, m]) => du.includes(y) && typeof m == "string"
        )
      )
    };
  } catch {
...
function Ht() {
  const g = vr.value.find((y) => y.id === Je.value.themeId);
  if (!g) return;
  const h = document.documentElement, o = { ...g.tokens, ...Je.value.overrides };
  for (const [y, m] of Object.entries(o))
    h.style.setProperty(`--${y}`, m);
  for (const [y, m] of Object.entries(Cm)) {
    const I = o[m];
    I && h.style.setProperty(`--${y}`, I);
  }
...
function pu() {
  ...
  function y(f) {
    vr.value.some((w) => w.id === f) && (Je.value = { themeId: f, overrides: {} }, fi(), Ht());
  }
  function m(f, w) {
    du.includes(f) && (Je.value = {
      ...Je.value,
      overrides: { ...Je.value.overrides, [f]: w }
    }, fi(), Ht());
  }
  function I(f) {
    const { [f]: w, ...K } = Je.value.overrides;
    Je.value = { ...Je.value, overrides: K }, fi(), Ht();
  }
  function c() {
    Je.value = { ...Je.value, overrides: {} }, fi(), Ht();
  }
  function E(f) {
    vr.value.some((w) => w.id === f.id) || (Yo.value = [...Yo.value, f]);
  }
  function C() {
    return {
      ...g.value,
  ...
  return {
    themes: vr,
    activeTheme: g,
    overrides: ne(() => Je.value.overrides),
    valueOf: h,
    isOverridden: o,
    selectTheme: y,
    setToken: m,
...
const $m = { class: "shell" }, Rm = {
  ...
  class: "rail",
  "aria-label": "Bereiche"
}, Tm = ["aria-current"], Om = ["aria-current"], Lm = ["aria-current", "title", "onClick"], Bm = { class: "sr-only" }, Dm = { class: "content" }, Wm = /* @__PURE__ */ Qe({
  __name: "App",
  setup(g) {
    const h = ee([]), { isLoading: o } = h_();
    pu();
    const y = br(), m = ea();
    Hn(() => {
      const f = xe(g_);
    ...
    const I = ne(
      () => y.params.pageid ? `/page/${y.params.pageid}` : "/page/abc"
    ), c = ne(() => String(y.name) === "home" && !y.params.pageid), E = ne(
      () => ["home", "page", "edit", "pageEdit"].includes(String(y.name))
    ), C = (f) => m.push(f);
    return (f, w) => {
      const K = u_("router-view");
      return _(), A("div", $m, [
        b(o) ? (_(), A("div", Rm, [...w[2] || (w[2] = [
        ...
        u("div", {
          ...
          c.value ? re("", !0) : (_(), A("nav", Em, [
            u("button", {
              type: "button",
              class: Ee(["ri", { on: E.value }]),
              "aria-current": E.value ? "page" : void 0,
              title: "Board",
              onClick: w[0] || (w[0] = (q) => C(I.value))
            }, [
              B(b(Xe), { name: "dashboard" }),
              w[3] || (w[3] = u("span", { class: "sr-only" }, "Board", -1))
            ], 10, Tm),
            u("button", {
              type: "button",
              class: Ee(["ri", { on: b(y).name === "data" }]),
              "aria-current": b(y).name === "data" ? "page" : void 0,
              title: "Verbindungen & Daten",
              onClick: w[1] || (w[1] = (q) => C("/datasources"))
            }, [
              B(b(Xe), { name: "dataset" }),
              w[4] || (w[4] = u("span", { class: "sr-only" }, "Verbindungen & Daten", -1))
            ], 10, Om),
            (_(!0), A(Ce, null, Ge(h.value, (q) => (_(), A("button", {
              key: q.id,
              type: "button",
              class: Ee(["ri", { on: b(y).name === q.routeName }]),
              "aria-current": b(y).name === q.routeName ? "page" : void 0,
              title: q.label,
              onClick: (G) => C(q.route)
            }, [
              B(b(Xe), {
...
}), Vm = /* @__PURE__ */ je(Wm, [["__scopeId", "data-v-27fec35c"]]), gu = "daanse.board.usage";
function Pm() {
  try {
...
function vu() {
  function g(m) {
    ...
    it.value = {
      ...it.value,
      [m]: { count: (I?.count ?? 0) + 1, lastOpened: Date.now() }
    };
    try {
  ...
  function o(m, I) {
    const c = it.value[m], E = it.value[I];
    return (E?.count ?? 0) !== (c?.count ?? 0) ? (E?.count ?? 0) - (c?.count ?? 0) : (E?.lastOpened ?? 0) - (c?.lastOpened ?? 0);
  }
  function y(m) {
    const I = it.value[m]?.lastOpened;
    if (!I) return "";
  ...
  return { usage: it, recordOpened: g, usageOf: h, byUsage: o, lastOpenedLabel: y };
}
const Mm = {
  key: 0,
  class: "loading-state"
...
}, zm = { class: "no-layout-message__text" }, qm = { key: 0 }, Gm = { key: 1 }, Hm = { key: 2 }, Km = /* @__PURE__ */ Qe({
  ...
  setup(g, { emit: h }) {
    const o = g, y = h, m = xe(yi), I = xe(yr), c = ee(null), E = ee(null), C = Bl(null), f = Bl(null), w = ee(0), K = ee(!0), q = (J) => {
      y("openWidgetSettings", J);
    }, G = (J) => {
      y("removeWidget", J);
    };
    let O = 0;
    const Y = async () => {
      const J = ++O;
      if (K.value = !0, await gi(), J === O) {
        if (o.pageId && I) {
          const ue = I.getPage(o.pageId);
          if (c.value = ue ?? null, ue?.layoutId && m) {
            const P = m.getLayout(ue.layoutId);
            E.value = P || null, o.viewMode ? (f.value = P?.component || null, C.value = null) : (C.value = P?.editor || null, f.value = null);
          } else
            C.value = null, f.value = null, E.value = null;
        }
        await new Promise((ue) => setTimeout(ue, 50)), J === O && (K.value = !1);
      }
    }, V = () => o.pageId ? I?.getPage(o.pageId) : void 0, $ = Yl(V);
    Nn($, () => {
      c.value = $.value ?? null;
    });
    const D = __(V, "layoutId");
    return Nn(D, () => {
      w.value++, Y();
    }), Hn(async () => {
      await gi(), await Y();
      ...
      K.value ? (_(), A("div", Mm, [...ue[0] || (ue[0] = [
        u("span", {
          class: "spinner",
          "aria-hidden": "true"
        }, null, -1),
        u("p", null, "Layout wird geladen…", -1)
      ])])) : o.viewMode && f.value && E.value ? (_(), A("div", Um, [
        (_(), Be(Kn(f.value), {
          key: E.value.id || "view",
          "layout-settings": c.value?.layoutSettings
        }, null, 8, ["layout-settings"]))
      ])) : !o.viewMode && C.value && E.value ? (_(), A("div", Nm, [
        (_(), Be(Kn(C.value), {
          key: E.value.id || "edit",
          "layout-settings": c.value?.layoutSettings,
          onOpenSettings: q,
        ...
        u("p", zm, [
          ...
          c.value ? E.value ? (_(), A("span", Hm, [
            ...
            u("small", null, "Layout: " + z(E.value.id), 1)
          ])) : (_(), A("span", Gm, " Für „" + z(c.value.name || o.pageId) + '" ist kein Layout eingestellt. ', 1)) : (_(), A("span", qm, " Die Seite „" + z(o.pageId) + '" gibt es nicht. ', 1))
        ])
      ]))
    ], 4));
  }
}), hu = /* @__PURE__ */ je(Km, [["__scopeId", "data-v-965fd8c7"]]), Ym = {
  class: "floorplan",
  "aria-hidden": "true"
}, Zm = {
  key: 0,
  ...
  setup(g) {
    ...
    const y = ne(() => {
      const m = (h.items ?? []).filter(
        (f) => Number.isFinite(f.x) && Number.isFinite(f.y)
      );
      if (m.length === 0) return [];
      const I = Math.max(...m.map((f) => (f.x ?? 0) + (f.width ?? 1))), c = Math.max(...m.map((f) => (f.y ?? 0) + (f.height ?? 1))), E = I > 0 ? I : 1, C = c > 0 ? c : 1;
      return m.map((f) => ({
        key: f.id ?? `${f.x}-${f.y}`,
        ...
        style: {
          ...
          width: `calc(${Math.max((f.width ?? 1) / E * 100, 3)}% - 3px)`,
          height: `calc(${Math.max((f.height ?? 1) / C * 100, 6)}% - 3px)`
        }
      }));
    });
    return (m, I) => (_(), A("div", Ym, [
      (_(!0), A(Ce, null, Ge(y.value, (c) => (_(), A("div", {
        key: c.key,
        class: Ee(["floorplan__block", `floorplan__block--${c.family}`]),
        style: Yt(c.style)
      }, null, 6))), 128)),
      y.value.length === 0 ? (_(), A("span", Zm, "keine Widgets")) : re("", !0)
    ]));
  }
}), Zo = /* @__PURE__ */ je(Jm, [["__scopeId", "data-v-0eecd73b"]]), { parse: Xm } = JSON, { keys: Qm } = Object, bi = String, jm = "string", Ul = {}, _u = "object", e1 = (g, h) => h, n1 = (g) => g instanceof bi ? bi(g) : g, t1 = (g, h) => typeof h === jm ? new bi(h) : h, r1 = (g, h, o, y) => (m) => {
  for (let I = Qm(m), { length: c } = I, E = 0; E < c; E++) {
    const C = I[E], f = m[C];
    if (f instanceof bi) {
      const w = g[+f];
      typeof w === _u && !o.has(w) ? (o.add(w), m[C] = Ul, h.push({ o: m, k: C, r: w })) : m[C] = y.call(m, C, w);
    } else m[C] !== Ul && (m[C] = y.call(m, C, f));
  }
  return m;
}, i1 = (g, h) => {
  const o = Xm(g, t1).map(n1), y = e1;
  let m = o[0];
  if (typeof m === _u && m) {
    const I = [], c = r1(o, I, /* @__PURE__ */ new Set(), y);
    m = c(m);
    let E = 0;
    for (; E < I.length; ) {
      const { o: C, k: f, r: w } = I[E++];
      C[f] = y.call(C, f, c(w));
    }
  }
  return y.call({ "": m }, "", m);
...
function s1(g) {
  ...
  const h = new R_(), o = /* @__PURE__ */ new Map();
  ...
  for (const I of g.datasources ?? []) {
    ...
    const E = o.get((I.config ?? {}).connection);
    E && (c.connection = E), h.datasources.push(c), y.set(I.uid, c);
  }
  const m = [];
  for (const I of g.variables ?? []) {
    const c = new O_();
    c.uid = I.id ?? I.uid ?? Math.random().toString(36).substring(7), c.name = I.name, c.type = I.type, c.scope = I.scope ?? "global", c.accessMode = I.accessMode ?? "external-writable", c.definition = I, h.variables.push(c), m.push({ entry: I, variable: c });
  }
  ...
  for (const [I, c] of Object.entries(g.pages ?? {})) {
    const E = c?.info ?? {}, C = new L_();
    C.id = E.id ?? I, C.name = E.name ?? "Seite", C.description = E.description, C.icon = E.icon, C.visibleInNavigation = E.visibleInNavigation ?? !0, C.layoutId = E.layoutId ?? E.layout?.id ?? "org.eclipse.daanse.board.app.ui.vue.layouts.base", C.layoutSettings = E.layoutSettings, C.backgroundColor = E.backgroundColor, C.backgroundImage = E.backgroundImage, C.backgroundSize = E.backgroundSize, C.backgroundPosition = E.backgroundPosition, C.backgroundRepeat = E.backgroundRepeat;
    for (const f of c?.widgets ?? []) {
      const w = new B_();
      w.uid = f.uid, w.type = f.type, w.config = f.config ?? {}, w.wrapperConfig = f.wrapperConfig ?? {};
      const K = y.get(f.config?.datasourceId);
      K && (w.datasource = K), C.widgets.push(w);
    }
    for (const f of c?.layout ?? []) {
      const w = new D_();
      w.id = f.id, w.x = f.x ?? 0, w.y = f.y ?? 0, w.z = f.z ?? 0, w.width = f.width ?? 300, w.height = f.height ?? 150, w.group = f.group, C.layout.push(w);
    }
    h.pages.push(C);
  }
  for (const { entry: I, variable: c } of m) {
    const E = I.pageId;
    E && (c.page = h.pages.toArray().find((C) => C.id === E));
  }
...
function d1() {
  const g = xe(at), h = xe(na), o = xe(ta), y = xe(eu), m = xe(o_), I = xe(s_);
  function c() {
    g.eventMappings.clear();
    for (const f of I?.getAllMappings() ?? []) {
      const w = new Xl();
      w.id = f.id, w.definition = f, g.eventMappings.push(w);
    }
  }
  function E() {
    c();
    ...
    f.getContents().add(g);
    const w = f.saveToString(/* @__PURE__ */ new Map([[W_, 2]]));
  ...
  function C(f) {
    ...
    if (!w) return [];
    g.connections.clear();
    for (const q of w.connections.toArray()) g.connections.push(q);
    g.datasources.clear();
    for (const q of w.datasources.toArray()) g.datasources.push(q);
    g.pages.clear();
    for (const q of w.pages.toArray()) g.pages.push(q);
    g.variables.clear();
    for (const q of w.variables.toArray()) g.variables.push(q);
    g.eventMappings.clear();
    for (const q of w.eventMappings.toArray()) g.eventMappings.push(q);
    g.defaultPage = g.pages.toArray().find((q) => q.id === w.defaultPage?.id) ?? (g.pages.size() > 0 ? g.pages.get(0) : void 0), h?.rebuildLive(), o?.rebuildLive(), y?.rebuildLive(), I?.setAllMappings(
      g.eventMappings.toArray().map((q) => q.definition)
    );
    const K = [];
    for (const q of g.pages.toArray())
      m?.initilazeVariableWrappers(
        q.widgets.toArray().map((G) => ({
          uid: G.uid,
          type: G.type,
          config: G.config,
...
function bu(g, h, o, y) {
  const m = Array.isArray(o) ? o : [], I = Array.isArray(y) ? y : [], c = {};
  for (const f of I)
    f?.uid && f.type && (c[f.uid] = f.type);
  const E = new Set(
    I.map((f) => f?.config?.datasourceId).filter((f) => !!f)
  ), C = [...new Set(I.map((f) => c1(f?.type ?? "")).filter(Boolean))];
  return {
    id: g,
...
const f1 = { class: "storage" }, p1 = {
  class: "tree",
  "aria-label": "Speicher"
}, g1 = {
  class: "tree__body",
  role: "tree"
}, v1 = ["aria-expanded", "onClick"], h1 = { class: "row__twist" }, _1 = { class: "row__name" }, m1 = { class: "row__meta" }, b1 = ["onClick"], y1 = { class: "row__name" }, w1 = { class: "row__meta" }, k1 = {
  key: 0,
  class: "row row--hint"
}, x1 = ["onClick"], S1 = {
  key: 0,
  ...
  role: "alert"
}, I1 = { class: "detail__head" }, $1 = { class: "detail__facts" }, R1 = { class: "create__hint" }, E1 = { class: "detail__head" }, T1 = { class: "detail__name" }, O1 = {
  key: 0,
  class: "detail__badge"
}, L1 = { class: "detail__facts" }, B1 = {
  key: 0,
  class: "boards"
}, D1 = { class: "board__text" }, W1 = { class: "board__name" }, V1 = { class: "board__facts" }, P1 = {
  key: 0,
  class: "board__kinds"
}, M1 = {
  key: 1,
  ...
  emits: ["restored"],
  setup(g, { emit: h }) {
    const o = h, y = xe(M_), { save: m, load: I } = d1(), c = ee([]), E = ee({}), C = ee(/* @__PURE__ */ new Set()), f = ee(), w = ee(), K = ee(""), q = ee(""), G = ee(!1), O = u1(), Y = ee(!1), V = ee("");
    function $(U) {
    ...
    function ve(U) {
      const k = P(U);
      if (!(!k || !y?.isViewForRepoType(k)))
        return y.getViewForRepoType(k);
    }
    const Ve = ne(() => ve(f.value));
    async function ie() {
      c.value = await y?.getAvailableReposetories() ?? [];
      const U = c.value[0];
      U && C.value.size === 0 && (f.value = U, await ce(U));
    }
    async function ce(U) {
      C.value.add(J(U)), C.value = new Set(C.value), await de(U);
    }
    async function de(U) {
      q.value = "", G.value = !0;
      try {
        E.value = { ...E.value, [J(U)]: await U.findAll() };
      } catch (k) {
        E.value = { ...E.value, [J(U)]: [] }, q.value = `Einträge konnten nicht gelesen werden: ${$(k)}`;
      } finally {
        G.value = !1;
      }
    }
    function $e(U) {
      f.value = U, Y.value = !1, C.value.has(J(U)) ? (C.value.delete(J(U)), C.value = new Set(C.value)) : ce(U);
    }
    function en(U) {
      const k = E.value[J(U)] ?? [], x = K.value.trim().toLowerCase();
      return [...x ? k.filter((Oe) => D(Oe).toLowerCase().includes(x)) : k].sort((Oe, Ae) => D(Oe).localeCompare(D(Ae)));
    }
    function ye(U, k) {
      f.value = U, w.value = k, Y.value = !1, q.value = "";
    }
    function F(U) {
      return String(U.uri) === O.value?.entryUri;
    }
    function N(U, k) {
      O.value = { placeUri: String(f.value?.uri), entryUri: String(U.uri), name: k };
    }
    function M(U) {
      return pe(Jo(U?.data));
    }
    ...
    const sn = ne(() => M(w.value)), ln = ne(() => ({
      boards: sn.value.length,
      widgets: sn.value.reduce((U, k) => U + k.widgetCount, 0),
      sources: sn.value.reduce((U, k) => U + k.sourceCount, 0)
    }));
    function un(U) {
      const k = M(U);
    ...
    const ht = ne(() => {
      if (!Y.value) return { boards: 0, widgets: 0 };
      const U = pe(Jo(m()));
      return {
        boards: U.length,
    ...
    async function $t(U) {
      q.value = "";
      try {
        const k = await f.value?.getEntityByUri(U.uri), x = I(k?.data ?? U.data);
        N(U, D(U)), o("restored", x);
      } catch (k) {
        q.value = `Laden fehlgeschlagen: ${$(k)}`;
      }
    }
    async function _t(U) {
      const k = f.value;
      if (k) {
        q.value = "";
        try {
          await k.update({ ...U, data: m() }), N(U, D(U)), await de(k), w.value = en(k).find((x) => String(x.uri) === String(U.uri));
        } catch (x) {
          q.value = `Speichern fehlgeschlagen: ${$(x)}`;
        }
      }
    }
    function On(U) {
      f.value = U, w.value = void 0, Y.value = !0, V.value = "", q.value = "";
    }
    async function Fn() {
      const U = f.value, k = V.value.trim();
      if (!(!U || !k)) {
        q.value = "";
        try {
          const x = { name: k, uri: ue(U, k), data: m() };
          await U.create(x), N(x, k), await ce(U), Y.value = !1, w.value = en(U).find((Z) => D(Z) === k);
        } catch (x) {
          q.value = `Anlegen fehlgeschlagen: ${$(x)}`;
        }
      }
    }
    async function Zt(U) {
      ...
      if (k) {
        q.value = "";
        try {
          await k.delete(U), F(U) && (O.value = void 0), w.value === U && (w.value = void 0), await de(k);
        } catch (x) {
          q.value = `Löschen fehlgeschlagen: ${$(x)}`;
        }
      }
    }
    function Yn(U) {
      const k = typeof U.data == "string" ? U.data : JSON.stringify(U.data), x = URL.createObjectURL(new Blob([k], { type: "application/json" })), Z = document.createElement("a");
      Z.href = x, Z.download = `${D(U)}.json`, Z.click(), URL.revokeObjectURL(x);
    }
    return Hn(ie), Nn(() => y, ie), (U, k) => (_(), A("div", f1, [
      u("aside", p1, [
        Kt(u("input", {
          "onUpdate:modelValue": k[0] || (k[0] = (x) => K.value = x),
          class: "tree__search",
          type: "search",
        ...
        u("div", g1, [
          (_(!0), A(Ce, null, Ge(c.value, (x) => (_(), A(Ce, {
            ...
            u("button", {
              type: "button",
              role: "treeitem",
              "aria-expanded": C.value.has(J(x)),
              class: Ee(["row", "row--place", { on: f.value === x && !w.value }]),
              onClick: (Z) => $e(x)
            }, [
              u("span", h1, z(C.value.has(J(x)) ? "▾" : "▸"), 1),
              u("span", _1, z(x.name), 1),
              u("span", m1, z((E.value[J(x)] ?? []).length), 1)
            ], 10, v1),
            C.value.has(J(x)) ? (_(), A(Ce, { key: 0 }, [
              (_(!0), A(Ce, null, Ge(en(x), (Z) => (_(), A("button", {
                key: String(Z.uri),
                type: "button",
                role: "treeitem",
                class: Ee(["row", "row--entry", { on: w.value === Z }]),
                onClick: (Oe) => ye(x, Z)
              }, [
                u("span", {
                  class: Ee(["row__dot", { open: F(Z) }]),
                  "aria-hidden": "true"
                }, null, 2),
                u("span", y1, z(D(Z)), 1),
                u("span", w1, z(un(Z)), 1)
              ], 10, b1))), 128)),
              en(x).length === 0 ? (_(), A("p", k1, z(K.value ? "Nichts gefunden" : "Noch kein Stand"), 1)) : re("", !0),
              Q(x) ? (_(), A("button", {
                key: 1,
                type: "button",
                class: "row row--add",
          ...
          c.value.length ? re("", !0) : (_(), A("p", S1, "Keine Speicherorte eingerichtet."))
        ])
      ]),
      u("section", C1, [
        q.value ? (_(), A("p", A1, z(q.value), 1)) : re("", !0),
        Y.value ? (_(), A(Ce, { key: 1 }, [
          u("header", I1, [
            k[9] || (k[9] = u("h2", { class: "detail__name" }, "Neuen Stand ablegen", -1)),
            u("span", $1, "in " + z(f.value?.name), 1)
          ]),
          u("form", {
            class: "create",
            ...
            u("p", R1, " Abgelegt wird der gesamte Arbeitsstand: " + z(ht.value.boards) + " " + z(ht.value.boards === 1 ? "Board" : "Boards") + " mit " + z(ht.value.widgets) + " Widgets, dazu Verbindungen, Datenquellen und Variablen. ", 1)
          ], 32)
        ], 64)) : Ve.value && !w.value ? (_(), Be(Kn(Ve.value), {
          key: 2,
          repo: f.value,
          context: E.value[J(f.value)],
          onClose: k[3] || (k[3] = (x) => de(f.value))
        }, null, 40, ["repo", "context"])) : w.value ? (_(), A(Ce, { key: 3 }, [
          u("header", E1, [
            u("h2", T1, z(D(w.value)), 1),
            F(w.value) ? (_(), A("span", O1, "geladen")) : re("", !0),
            u("span", L1, z(f.value?.name) + " · " + z(ln.value.boards) + " " + z(ln.value.boards === 1 ? "Board" : "Boards") + " · " + z(ln.value.widgets) + " Widgets · " + z(ln.value.sources) + " Datenquellen ", 1),
            k[17] || (k[17] = u("span", { class: "detail__spacer" }, null, -1)),
            B(b(be), {
              intent: "primary",
              size: "sm",
              onClick: k[4] || (k[4] = (x) => $t(w.value))
            }, {
          ...
          sn.value.length ? (_(), A("div", B1, [
            (_(!0), A(Ce, null, Ge(sn.value, (x) => (_(), A("article", {
              ...
              u("div", D1, [
                u("h3", W1, z(x.name), 1),
                u("p", V1, z(x.widgetCount) + " Widgets · " + z(x.sourceCount) + " " + z(x.sourceCount === 1 ? "Datenquelle" : "Datenquellen"), 1),
                x.kinds.length ? (_(), A("p", P1, z(x.kinds.join(" · ")), 1)) : re("", !0)
              ])
            ]))), 128))
          ])) : (_(), A("p", M1, " In diesem Stand liegt kein Board - er enthält nur Verbindungen, Quellen oder Variablen. "))
        ], 64)) : G.value ? (_(), A("p", U1, "Wird gelesen…")) : (_(), A("p", N1, "Wähle links einen Stand, um zu sehen, was darin liegt."))
      ])
    ]));
  }
}), z1 = /* @__PURE__ */ je(F1, [["__scopeId", "data-v-82d034f3"]]), q1 = { class: "boards" }, G1 = { class: "boards__panel" }, H1 = { class: "boards__bar" }, K1 = {
  class: "boards__views",
  role: "tablist",
  "aria-label": "Ansicht"
}, Y1 = ["aria-selected"], Z1 = {
  ...
  key: 0,
  class: "boards__tools"
}, Q1 = { class: "boards__body" }, j1 = {
  key: 0,
  class: "boards__empty"
}, e0 = { class: "boards__empty-actions" }, n0 = {
  key: 1,
  class: "boards__grid"
}, t0 = ["aria-label", "onClick", "onKeydown"], r0 = { class: "board__body" }, i0 = { class: "board__name" }, o0 = { class: "board__meta" }, a0 = {
  key: 0,
  class: "board__usage"
}, s0 = {
  key: 1,
  ...
  setup(g) {
    const h = g, o = ea(), y = br(), m = ee(""), { usageOf: I, byUsage: c, lastOpenedLabel: E } = vu(), C = ee(y.query.view === "storage" ? "storage" : "boards"), f = Tn(xe(at), (V) => V.pages), w = ne(() => {
      f.value;
      const V = h.pageRepo;
      return V ? V.getAllPageIds().slice().sort(c).map(
        ($) => (
          ...
          bu(
            $,
            V.getPage($),
            V.getPage($)?.layout?.toArray() ?? [],
            V.getPage($)?.widgets?.toArray() ?? []
          )
        )
      ) : [];
    }), K = ne(() => {
      ...
      return V ? w.value.filter(
        ($) => $.name.toLowerCase().includes(V) || $.description.toLowerCase().includes(V) || $.kinds.some((D) => D.toLowerCase().includes(V))
      ) : w.value;
    });
    function q(V) {
      o.push(`/page/${V}`);
    ...
    return (V, $) => (_(), A("div", q1, [
      u("div", G1, [
        u("header", H1, [
          u("div", K1, [
            u("button", {
              ...
              onClick: $[0] || ($[0] = (D) => C.value = "boards")
            }, [
              $[4] || ($[4] = ae(" Oft genutzt ", -1)),
              w.value.length ? (_(), A("span", Z1, z(w.value.length), 1)) : re("", !0)
            ], 10, Y1),
            u("button", {
              type: "button",
              role: "tab",
              ...
              onClick: $[1] || ($[1] = (D) => C.value = "storage")
            }, " Speicher ", 10, J1)
          ]),
          C.value === "boards" ? (_(), A("div", X1, [
            Kt(u("input", {
              "onUpdate:modelValue": $[2] || ($[2] = (D) => m.value = D),
        ...
        u("div", Q1, [
          C.value === "boards" && w.value.length === 0 ? (_(), A("div", j1, [
            B(Zo, {
            ...
            $[8] || ($[8] = u("h2", { class: "boards__empty-title" }, "Noch kein Board", -1)),
            $[9] || ($[9] = u("p", { class: "boards__empty-text" }, " Ein Board ist eine Seite mit Widgets über deinen Datenquellen. Lege eines an oder öffne einen gespeicherten Arbeitsstand. ", -1)),
            u("div", e0, [
              B(b(be), {
                intent: "primary",
                size: "sm",
          ...
          ])) : C.value === "boards" ? (_(), A("div", n0, [
            (_(!0), A(Ce, null, Ge(K.value, (D) => (_(), A("article", {
              key: D.id,
              class: "board",
              tabindex: "0",
              role: "button",
              ...
              u("div", r0, [
                ...
                b(I)(D.id) ? (_(), A("p", a0, z(b(I)(D.id)?.count) + "× geöffnet · zuletzt " + z(b(E)(D.id)), 1)) : re("", !0),
                D.kinds.length ? (_(), A("ul", s0, [
                  (_(!0), A(Ce, null, Ge(D.kinds.slice(0, 3), (J) => (_(), A("li", {
                    key: J,
                    class: "board__kind"
                  }, z(J), 1))), 128)),
                  D.kinds.length > 3 ? (_(), A("li", l0, " +" + z(D.kinds.length - 3), 1)) : re("", !0)
                ])) : re("", !0)
              ]),
              u("button", {
                class: "board__edit",
            ...
            K.value.length === 0 ? (_(), A("p", d0, " Kein Board passt zu „" + z(m.value) + "“. ", 1)) : re("", !0)
          ])) : (_(), Be(z1, {
            key: 2,
            onRestored: $[3] || ($[3] = (D) => C.value = "boards")
          }))
        ])
      ])
    ]));
  }
}), f0 = /* @__PURE__ */ je(c0, [["__scopeId", "data-v-4c11f243"]]), p0 = { class: "report-container" }, g0 = /* @__PURE__ */ Qe({
  __name: "ViewReport",
  props: ["params"],
  setup(g) {
    const h = g, o = br(), y = xe(yr), m = xe(yi), I = ne(() => h.params?.pageid ?? o.params.pageid ?? ""), { recordOpened: c } = vu();
    return Nn(I, (E) => c(E), { immediate: !0 }), (E, C) => (_(), A("div", p0, [
      I.value ? (_(), Be(hu, {
        key: 0,
        pageId: I.value,
        viewMode: !0
      }, null, 8, ["pageId"])) : (_(), Be(f0, {
        key: 1,
        "page-repo": b(y),
        "layout-repo": b(m)
...
}), Nl = /* @__PURE__ */ je(g0, [["__scopeId", "data-v-3ecae506"]]), v0 = { boards: 0, widgets: 0 };
function yu() {
  const g = xe(yr);
  function h() {
    const m = {};
    ...
    for (const I of g.getAllPageIds()) {
      ...
      for (const E of g.getPage(I)?.widgets?.toArray() ?? []) {
        const C = E.datasource?.uid;
        if (!C) continue;
        const f = m[C] ??= { boards: 0, widgets: 0 };
        f.widgets++, c.add(C);
      }
      for (const E of c) m[E].boards++;
    }
    return m;
  }
  function o(m) {
  ...
  function y(m) {
    if (!m.widgets) return "";
    const I = `${m.boards} ${m.boards === 1 ? "Board" : "Boards"}`, c = `${m.widgets} ${m.widgets === 1 ? "Widget" : "Widgets"}`;
    return `${I} · ${c}`;
  }
  return { usageByDatasource: h, usageOf: o, usageLabel: y };
}
const h0 = { class: "tree" }, _0 = { class: "tree__head" }, m0 = { class: "tree__search" }, b0 = { class: "tree__body" }, y0 = {
  key: 0,
  class: "tree__empty"
}, w0 = {
  key: 1,
  class: "tree__list"
}, k0 = ["title", "onClick"], x0 = ["disabled", "onClick"], S0 = { class: "row__name" }, C0 = {
  key: 0,
  class: "row__what"
}, A0 = { class: "row__what" }, I0 = {
  key: 0,
  class: "tree__sources"
}, $0 = { class: "row__twist row__twist--none" }, R0 = ["onClick"], E0 = { class: "row__name" }, T0 = { class: "row__what" }, O0 = {
  key: 0,
  class: "row__usage"
}, L0 = {
  key: 0,
  class: "tree__none"
}, B0 = { class: "confirm__title" }, D0 = { class: "confirm__text" }, W0 = {
  key: 0,
  ...
  setup(g, { emit: h }) {
    const o = jo(g, "modelValue"), y = xe(na), m = xe(at), I = Tn(m, (ie) => ie.connections), c = xe(ta), E = Tn(m, (ie) => ie.datasources), { usageByDatasource: C, usageLabel: f } = yu(), w = h, K = ee(""), q = ee(/* @__PURE__ */ new Set()), G = ne(() => {
      const ie = C(), ce = K.value.trim().toLowerCase(), de = (...M) => !ce || M.some((pe) => (pe ?? "").toLowerCase().includes(ce)), $e = (M) => E.value.filter((pe) => pe.connection?.uid === M).filter((pe) => de(pe.name, pe.type, pe.uid)).map((pe) => ({
        uid: pe.uid,
        name: pe.name,
        type: pe.type,
      ...
      })), ye = new Set(I.value.map((M) => M.uid)), F = E.value.filter((M) => !M.connection || !ye.has(M.connection.uid)).filter((M) => de(M.name, M.type, M.uid)).map((M) => ({
        uid: M.uid,
        name: M.name,
        type: M.type,
    ...
    function Y(ie) {
      const ce = new Set(q.value);
      ce.has(ie) ? ce.delete(ie) : ce.add(ie), q.value = ce;
    }
    ...
    function D() {
      $("Connection", y.createConnection("").uid);
    }
    function J() {
      $("DataSource", c.createDatasource("").uid);
    }
    const ue = ee(void 0);
    function P(ie, ce) {
    ...
    function Q() {
      const ie = ue.value;
      ie && (ie.type === "Connection" ? y.removeConnection(ie.itemId) : c.removeDatasource(ie.itemId), V(ie.type, ie.itemId) && (o.value = void 0), ue.value = void 0);
    }
    const ve = ne(() => {
      const ie = ue.value;
      return ie ? (ie.type === "Connection" ? I.value.find((de) => de.uid === ie.itemId) : E.value.find((de) => de.uid === ie.itemId))?.name ?? ie.itemId : "";
    }), Ve = ne(() => {
      const ie = ue.value;
    ...
    return (ie, ce) => (_(), A(Ce, null, [
      u("section", h0, [
        u("header", _0, [
          ...
          B(b(be), {
            ...
            onClick: ce[0] || (ce[0] = (de) => w("findEndpoints"))
          }, {
            default: te(() => [
              B(b(Xe), {
                name: "travel_explore",
        ...
        u("div", m0, [
          B(b(gn), {
            modelValue: K.value,
            "onUpdate:modelValue": ce[1] || (ce[1] = (de) => K.value = de),
            type: "search",
            placeholder: "Suchen…",
            stacked: ""
          }, null, 8, ["modelValue"])
        ...
        u("div", b0, [
          G.value.length ? (_(), A("ul", w0, [
            (_(!0), A(Ce, null, Ge(G.value, (de) => (_(), A("li", {
              key: de.uid || "loose"
            }, [
              u("div", {
                ...
                u("button", {
                  type: "button",
                  class: "row__twist",
                  title: O(de.uid) ? "Zuklappen" : "Aufklappen",
                  onClick: ($e) => Y(de.uid)
                }, [
                  B(b(Xe), {
                    name: O(de.uid) ? "expand_more" : "chevron_right",
                    size: "sm"
                  }, null, 8, ["name"])
                ], 8, k0),
                u("button", {
                  ...
                  class: "row__body",
                  disabled: de.orphan,
                  onClick: ($e) => !de.orphan && $("Connection", de.uid)
                }, [
                  u("span", S0, z(de.name), 1),
                  de.type ? (_(), A("span", C0, z(de.type), 1)) : re("", !0),
                  u("span", A0, z(de.sources.length), 1)
                ], 8, x0),
                de.orphan ? re("", !0) : (_(), Be(b(be), {
                  key: 0,
                  intent: "quiet",
                  size: "sm",
              ...
              O(de.uid) ? (_(), A("ul", I0, [
                (_(!0), A(Ce, null, Ge(de.sources, ($e) => (_(), A("li", {
                  ...
                  u("div", {
                    ...
                    u("button", {
                      type: "button",
                      class: "row__body",
                      onClick: (en) => $("DataSource", $e.uid)
                    }, [
                      u("span", E0, z($e.name), 1),
                      u("span", T0, z($e.type), 1),
                      $e.usage ? (_(), A("span", O0, z($e.usage), 1)) : re("", !0)
                    ], 8, R0),
                    B(b(be), {
                      intent: "quiet",
                      size: "sm",
                ...
                !de.sources.length && !de.orphan ? (_(), A("li", L0, " Keine Datenquelle an dieser Verbindung. ")) : re("", !0)
              ])) : re("", !0)
            ]))), 128))
          ])) : (_(), A("p", y0, [
            K.value ? (_(), A(Ce, { key: 0 }, [
              ae("Nichts gefunden.")
            ], 64)) : (_(), A(Ce, { key: 1 }, [
      ...
      B(b(Ko), {
        ...
        header: te(() => [
          ...
          u("h2", B0, z(ue.value?.type === "Connection" ? "Verbindung löschen" : "Datenquelle löschen"), 1)
        ]),
        actions: te(() => [
          B(b(be), {
            intent: "quiet",
        ...
        default: te(() => [
          u("p", D0, " „" + z(ve.value) + "“ wird entfernt. Das lässt sich nicht rückgängig machen. ", 1),
          Ve.value ? (_(), A("p", W0, " Verwendet in " + z(Ve.value) + " — die lesen danach ins Leere. ", 1)) : re("", !0)
        ]),
        _: 1
      }, 8, ["model-value"])
    ], 64));
  }
}), P0 = /* @__PURE__ */ je(V0, [["__scopeId", "data-v-19322c5a"]]);
 ...
 */
var M0 = hr.exports, Fl;
function U0() {
  return Fl || (Fl = 1, (function(g, h) {
    (function() {
      var o, y = "4.17.21", m = 200, I = "Unsupported core-js use. Try https://npms.io/search?q=ponyfill.", c = "Expected a function", E = "Invalid `variable` option passed into `_.template`", C = "__lodash_hash_undefined__", f = 500, w = "__lodash_placeholder__", K = 1, q = 2, G = 4, O = 1, Y = 2, V = 1, $ = 2, D = 4, J = 8, ue = 16, P = 32, Q = 64, ve = 128, Ve = 256, ie = 512, ce = 30, de = "...", $e = 800, en = 16, ye = 1, F = 2, N = 3, M = 1 / 0, pe = 9007199254740991, sn = 17976931348623157e292, ln = NaN, un = 4294967295, ht = un - 1, $t = un >>> 1, _t = [
        ["ary", ve],
        ["bind", V],
        ["bindKey", $],
        ["curry", J],
      ...
      We[wi] = We[ki] = We[xi] = We[Si] = We[Ci] = We[Ai] = We[Ii] = We[$i] = We[Ri] = !0, We[On] = We[Fn] = We[jt] = We[Yn] = We[Rt] = We[U] = We[x] = We[Z] = We[Ae] = We[kn] = We[Ln] = We[Jt] = We[Bn] = We[Xt] = We[Qt] = !1;
      var De = {};
      De[On] = De[Fn] = De[jt] = De[Rt] = De[Yn] = De[U] = De[wi] = De[ki] = De[xi] = De[Si] = De[Ci] = De[Ae] = De[kn] = De[Ln] = De[Jt] = De[Bn] = De[Xt] = De[wr] = De[Ai] = De[Ii] = De[$i] = De[Ri] = !0, De[x] = De[Z] = De[Qt] = !1;
      var bd = {
      ...
      function hn(p, R, S) {
        switch (S.length) {
          ...
          case 1:
            return p.call(R, S[0]);
          case 2:
            return p.call(R, S[0], S[1]);
          case 3:
            return p.call(R, S[0], S[1], S[2]);
        }
        return p.apply(R, S);
      }
      function Ad(p, R, S, X) {
      ...
      function Sn(p, R) {
        for (var S = -1, X = p == null ? 0 : p.length; ++S < X && R(p[S], S, p) !== !1; )
          ;
        return p;
      }
      function Id(p, R) {
        for (var S = p == null ? 0 : p.length; S-- && R(p[S], S, p) !== !1; )
          ;
        return p;
      }
      function Ba(p, R) {
        for (var S = -1, X = p == null ? 0 : p.length; ++S < X; )
          if (!R(p[S], S, p))
            return !1;
        return !0;
      }
      function st(p, R) {
        for (var S = -1, X = p == null ? 0 : p.length, fe = 0, Ie = []; ++S < X; ) {
          var ze = p[S];
          R(ze, S, p) && (Ie[fe++] = ze);
        }
        return Ie;
      ...
      function Mi(p, R, S) {
        for (var X = -1, fe = p == null ? 0 : p.length; ++X < fe; )
          if (S(R, p[X]))
            return !0;
        return !1;
      }
      function Pe(p, R) {
        for (var S = -1, X = p == null ? 0 : p.length, fe = Array(X); ++S < X; )
          fe[S] = R(p[S], S, p);
        return fe;
      }
      function lt(p, R) {
        for (var S = -1, X = R.length, fe = p.length; ++S < X; )
          p[fe + S] = R[S];
        return p;
      }
      function Ui(p, R, S, X) {
        var fe = -1, Ie = p == null ? 0 : p.length;
        for (X && Ie && (S = p[++fe]); ++fe < Ie; )
          S = R(S, p[fe], fe, p);
        return S;
      }
      function $d(p, R, S, X) {
      ...
      function Ar(p, R, S, X) {
        for (var fe = p.length, Ie = S + (X ? 1 : -1); X ? Ie-- : ++Ie < fe; )
          if (R(p[Ie], Ie, p))
            return Ie;
        return -1;
      }
      function Tt(p, R, S) {
        return R === R ? zd(p, R, S) : Ar(p, Wa, S);
      }
      function Od(p, R, S, X) {
      ...
      function Pa(p, R, S, X, fe) {
        return fe(p, function(Ie, ze, Le) {
          S = X ? (X = !1, Ie) : R(S, Ie, ze, Le);
        }), S;
      }
      function Ld(p, R) {
        var S = p.length;
      ...
      function qi(p, R) {
        for (var S, X = -1, fe = p.length; ++X < fe; ) {
          var Ie = R(p[X]);
          Ie !== o && (S = S === o ? Ie : S + Ie);
        }
        return S;
      ...
      function Ua(p, R) {
        for (var S = -1, X = p.length; ++S < X && Tt(R, p[S], 0) > -1; )
          ;
        return S;
      }
      function Na(p, R) {
        for (var S = p.length; S-- && Tt(R, p[S], 0) > -1; )
          ;
        return S;
      }
      function Dd(p, R) {
        for (var S = p.length, X = 0; S--; )
          p[S] === R && ++X;
        return X;
      }
      ...
      function Nd(p) {
        for (var R, S = []; !(R = p.next()).done; )
          S.push(R.value);
        return S;
      }
      function Ki(p) {
        var R = -1, S = Array(p.size);
        return p.forEach(function(X, fe) {
          S[++R] = [fe, X];
        }), S;
      }
      function Fa(p, R) {
      ...
      function ut(p, R) {
        for (var S = -1, X = p.length, fe = 0, Ie = []; ++S < X; ) {
          var ze = p[S];
          (ze === R || ze === w) && (p[S] = w, Ie[fe++] = S);
        }
        return Ie;
      }
      function Ir(p) {
      ...
      function zd(p, R, S) {
        for (var X = S - 1, fe = p.length; ++X < fe; )
          if (p[X] === R)
            return X;
        return -1;
      }
      ...
      function za(p) {
        for (var R = p.length; R-- && Du.test(p.charAt(R)); )
          ;
        return R;
      }
      var Gd = zi(wd);
      function Hd(p) {
        for (var R = Wi.lastIndex = 0; Wi.test(p); )
          ++R;
        return R;
      }
      function Kd(p) {
      ...
      var Zd = (function p(R) {
        R = R == null ? Ke : Bt.defaults(Ke.Object(), R, Bt.pick(Ke, _d));
        var S = R.Array, X = R.Date, fe = R.Error, Ie = R.Function, ze = R.Math, Le = R.Object, Yi = R.RegExp, Jd = R.String, Cn = R.TypeError, $r = S.prototype, Xd = Ie.prototype, Dt = Le.prototype, Rr = R["__core-js_shared__"], Er = Xd.toString, Te = Dt.hasOwnProperty, Qd = 0, qa = (function() {
        ...
        ), Or = Ia ? R.Buffer : o, dt = R.Symbol, Lr = R.Uint8Array, Ga = Or ? Or.allocUnsafe : o, Br = Fa(Le.getPrototypeOf, Le), Ha = Le.create, Ka = Dt.propertyIsEnumerable, Dr = $r.splice, Ya = dt ? dt.isConcatSpreadable : o, nr = dt ? dt.iterator : o, bt = dt ? dt.toStringTag : o, Wr = (function() {
          try {
            var e = St(Le, "defineProperty");
            return e({}, "", {}), e;
          } catch {
          }
        })(), tc = R.clearTimeout !== Ke.clearTimeout && R.clearTimeout, rc = X && X.now !== Ke.Date.now && X.now, ic = R.setTimeout !== Ke.setTimeout && R.setTimeout, Vr = ze.ceil, Pr = ze.floor, Zi = Le.getOwnPropertySymbols, oc = Or ? Or.isBuffer : o, Za = R.isFinite, ac = $r.join, sc = Fa(Le.keys, Le), qe = ze.max, nn = ze.min, lc = X.now, uc = R.parseInt, Ja = ze.random, dc = $r.reverse, Ji = St(R, "DataView"), tr = St(R, "Map"), Xi = St(R, "Promise"), Wt = St(R, "Set"), rr = St(R, "WeakMap"), ir = St(Le, "create"), Mr = rr && new rr(), Vt = {}, cc = Ct(Ji), fc = Ct(tr), pc = Ct(Xi), gc = Ct(Wt), vc = Ct(rr), Ur = dt ? dt.prototype : o, or = Ur ? Ur.valueOf : o, Xa = Ur ? Ur.toString : o;
        function a(e) {
        ...
        function mc() {
          var e = this.__wrapped__.value(), n = this.__dir__, t = ge(e), r = n < 0, i = t ? e.length : 0, s = Tf(0, i, this.__views__), l = s.start, d = s.end, v = d - l, T = r ? d : l - 1, L = this.__iteratees__, W = L.length, H = 0, j = nn(v, this.__takeCount__);
          if (!t || !r && i == v && j == v)
        ...
        Zn.prototype.clear = Sc, Zn.prototype.delete = Cc, Zn.prototype.get = Ac, Zn.prototype.has = Ic, Zn.prototype.set = $c;
        function Jn(e) {
          var n = -1, t = e == null ? 0 : e.length;
          for (this.clear(); ++n < t; ) {
            var r = e[n];
        ...
        Jn.prototype.clear = Rc, Jn.prototype.delete = Ec, Jn.prototype.get = Tc, Jn.prototype.has = Oc, Jn.prototype.set = Lc;
        function wt(e) {
          var n = -1, t = e == null ? 0 : e.length;
        ...
        function Qa(e, n) {
          ...
          for (var T in e)
            (n || Te.call(e, T)) && !(l && // Safari 9 has enumerable `arguments.length` in strict mode.
        ...
        function In(e, n, t, r, i, s) {
          ...
          var se = T ? v ? yo : bo : v ? fn : He, _e = L ? o : se(e);
        ...
        function sr(e, n, t, r) {
          var i = -1, s = Cr, l = !0, d = e.length, v = [], T = n.length;
          if (!d)
            return v;
          t && (n = Pe(n, _n(t))), r ? (s = Mi, l = !1) : n.length >= m && (s = er, l = !1, n = new wt(n));
          e:
            for (; ++i < d; ) {
        ...
        function ro(e, n, t) {
          for (var r = t ? Mi : Cr, i = e[0].length, s = e.length, l = s, d = S(s), v = 1 / 0, T = []; l--; ) {
            var L = e[l];
          ...
          e:
            for (; ++W < i && T.length < v; ) {
              var j = L[W], se = n ? n(j) : j;
              if (j = t || j !== 0 ? j : 0, !(H ? er(H, se) : r(T, se, t))) {
                for (l = s; --l; ) {
                  var _e = d[l];
                  if (!(_e ? er(_e, se) : r(e[l], se, t)))
                    continue e;
                }
                H && H.push(se), T.push(j);
              }
            }
          return T;
        }
        ...
        function ef(e, n, t, r, i, s) {
          var l = ge(e), d = ge(n), v = l ? Fn : tn(e), T = d ? Fn : tn(n);
          v = v == On ? Ln : v, T = T == On ? Ln : T;
          var L = v == Ln, W = T == Ln, H = v == T;
          if (H && vt(e)) {
            if (!vt(n))
              return !1;
            l = !0, L = !1;
          ...
          if (!(t & O)) {
            var j = L && Te.call(e, "__wrapped__"), se = W && Te.call(n, "__wrapped__");
            if (j || se) {
              var _e = j ? e.value() : e, le = se ? n.value() : n;
              return s || (s = new Wn()), i(_e, le, t, r, s);
            }
          }
          return H ? (s || (s = new Wn()), Rf(e, n, t, r, i, s)) : !1;
        }
        function nf(e) {
          return Ue(e) && tn(e) == Ae;
        }
        ...
        function fs(e, n) {
          return So(e) && qs(n) ? Gs(Gn(e), n) : function(t) {
            var r = Bo(t, e);
            return r === o && r === n ? Do(t, e) : ur(n, r, O | Y);
          };
        }
        ...
        function sf(e, n, t, r, i, s, l) {
          var d = Ao(e, t), v = Ao(n, t), T = l.get(v);
          if (T) {
            Qi(e, t, T);
            return;
          }
          ...
          if (W) {
            var H = ge(v), j = !H && vt(v), se = !H && !j && Ft(v);
            L = v, H || j || se ? ge(d) ? L = d : Ne(d) ? L = dn(d) : j ? (W = !1, L = Ss(v, !0)) : se ? (W = !1, L = Cs(v, !0)) : L = [] : gr(v) || At(v) ? (L = d, At(d) ? L = wl(d) : (!Me(d) || nt(d)) && (L = zs(v))) : W = !1;
          }
        ...
        function so(e, n, t, r) {
          ...
          for (e === n && (n = dn(n)), t && (d = Pe(e, _n(t))); ++s < l; )
            for (var v = 0, T = n[s], L = t ? t(T) : T; (v = i(d, L, v, r)) > -1; )
        ...
        function dr(e, n, t, r) {
          ...
          for (var i = -1, s = n.length, l = s - 1, d = e; d != null && ++i < s; ) {
            ...
            if (i != l) {
              var L = d[v];
              T = r ? r(L, v, d) : o, T === o && (T = Me(L) ? L : et(n[i + 1]) ? [] : {});
            }
            ar(d, v, T), d = d[v];
          }
          return e;
        ...
        function co(e, n, t, r) {
          ...
          if (s === 0)
            return 0;
          n = t(n);
          for (var l = n !== n, d = n === null, v = bn(n), T = n === o; i < s; ) {
            var L = Pr((i + s) / 2), W = t(e[L]), H = W !== o, j = W === null, se = W === W, _e = bn(W);
            if (l)
              var le = r || se;
            else T ? le = se && (r || H) : d ? le = se && H && (r || !j) : v ? le = se && H && !j && (r || !_e) : j || _e ? le = !1 : le = r ? W <= n : W < n;
            le ? i = L + 1 : s = L;
          }
          return nn(s, ht);
        }
        ...
        function As(e, n) {
          if (e !== n) {
            var t = e !== o, r = e === null, i = e === e, s = bn(e), l = n !== o, d = n === null, v = n === n, T = bn(n);
            if (!d && !T && !s && e > n || s && l && v && !d && !T || r && l && v || !t && v || !i)
              return 1;
            if (!r && !s && !T && e < n || T && t && i && !r && !s || d && t && i || !l && i || !v)
              return -1;
          }
          return 0;
        }
        ...
        function Is(e, n, t, r) {
          for (var i = -1, s = e.length, l = t.length, d = -1, v = n.length, T = qe(s - l, 0), L = S(v + T), W = !r; ++d < v; )
            L[d] = n[d];
          for (; ++i < l; )
            (W || i < s) && (L[t[i]] = e[i]);
          for (; T--; )
        ...
        function $s(e, n, t, r) {
          for (var i = -1, s = e.length, l = -1, d = t.length, v = -1, T = n.length, L = qe(s - d, 0), W = S(L + T), H = !r; ++i < L; )
            W[i] = e[i];
          for (var j = i; ++v < T; )
            W[j + v] = n[v];
          for (; ++l < d; )
        ...
        function Sf(e, n, t) {
          ...
          function i() {
            for (var s = arguments.length, l = S(s), d = s, v = Nt(i); d--; )
              l[d] = arguments[d];
            var T = s < 3 && l[0] !== v && l[s - 1] !== v ? [] : ut(l, v);
            if (s -= T.length, s < t)
              return Ws(
                e,
                n,
        ...
        function Ls(e) {
          return jn(function(n) {
            ...
            for (r = l ? r : t; ++r < t; ) {
              ...
              v && Co(v[0]) && v[1] == (ve | J | P | Ve) && !v[4].length && v[9] == 1 ? l = l[jr(v[0])].apply(l, v[3]) : l = s.length == 1 && Co(s) ? l[d]() : l.thru(s);
            }
            return function() {
              var T = arguments, L = T[0];
              if (l && T.length == 1 && ge(L))
                return l.plant(L).value();
              for (var W = 0, H = t ? n[W].apply(this, T) : L; ++W < t; )
                H = n[W].call(this, H);
              return H;
            };
          });
        }
        function Zr(e, n, t, r, i, s, l, d, v, T) {
          var L = n & ve, W = n & V, H = n & $, j = n & (J | ue), se = n & ie, _e = H ? o : cr(e);
          function le() {
            for (var we = arguments.length, Se = S(we), yn = we; yn--; )
              Se[yn] = arguments[yn];
            if (j)
              var an = Nt(le), wn = Dd(Se, an);
            if (r && (Se = Is(Se, r, i, j)), s && (Se = $s(Se, s, l, j)), we -= wn, j && we < T) {
              var Fe = ut(Se, an);
              return Ws(
                e,
                n,
            ...
            var Pn = W ? t : this, rt = H ? Pn[e] : e;
            return we = Se.length, d ? Se = qf(Se, d) : se && we > 1 && Se.reverse(), L && v < we && (Se.length = v), this && this !== Ke && this instanceof le && (rt = _e || cr(rt)), rt.apply(Pn, Se);
          }
          return le;
        }
        function Bs(e, n) {
        ...
        function Cf(e, n, t, r) {
          var i = n & V, s = cr(e);
          function l() {
            for (var d = -1, v = arguments.length, T = -1, L = r.length, W = S(L + v), H = this && this !== Ke && this instanceof l ? s : e; ++T < L; )
              W[T] = r[T];
            for (; v--; )
              W[T++] = arguments[++d];
            return hn(H, i ? t : this, W);
          }
          return l;
        }
        function Ds(e) {
          return function(n, t, r) {
            return r && typeof r != "number" && on(n, t, r) && (t = r = o), n = tt(n), t === o ? (t = n, n = 0) : t = tt(t), r = r === o ? n < t ? 1 : -1 : tt(r), df(n, t, r, e);
          };
        }
        function Qr(e) {
          return function(n, t) {
        ...
        function Ws(e, n, t, r, i, s, l, d, v, T) {
          var L = n & J, W = L ? l : o, H = L ? o : l, j = L ? s : o, se = L ? o : s;
          n |= L ? P : Q, n &= ~(L ? Q : P), n & D || (n &= -4);
          var _e = [
            e,
            n,
            i,
        ...
        function Qn(e, n, t, r, i, s, l, d) {
          ...
          if (!v && typeof e != "function")
            throw new Cn(c);
          var T = r ? r.length : 0;
          if (T || (n &= -97, r = i = o), l = l === o ? l : qe(he(l), 0), d = d === o ? d : he(d), T -= i ? i.length : 0, n & Q) {
            var L = r, W = i;
            r = i = o;
          }
          var H = v ? o : wo(e), j = [
          ...
          if (H && Nf(j, H), e = j[0], n = j[1], t = j[2], r = j[3], i = j[4], d = j[9] = j[9] === o ? v ? 0 : e.length : qe(j[9] - T, 0), !d && n & (J | ue) && (n &= -25), !n || n == V)
            var se = xf(e, n, t);
          else n == J || n == ue ? se = Sf(e, n, d) : (n == P || n == (V | P)) && !i.length ? se = Cf(e, n, t, r) : se = Zr.apply(o, j);
          var _e = H ? _s : Ys;
          return Zs(_e(se, j), e, n);
        }
        function Ps(e, n, t, r) {
        ...
        function Us(e, n, t, r, i, s) {
          var l = t & O, d = e.length, v = n.length;
          if (d != v && !(l && v > d))
            return !1;
          var T = s.get(e), L = s.get(n);
          if (T && L)
            return T == n && L == e;
          var W = -1, H = !0, j = t & Y ? new wt() : o;
          for (s.set(e, n), s.set(n, e); ++W < d; ) {
            ...
            if (j) {
              if (!Ni(n, function(we, Se) {
                if (!er(j, Se) && (se === we || i(se, we, t, r, s)))
                  return j.push(Se);
              })) {
                H = !1;
                break;
        ...
        function Rf(e, n, t, r, i, s) {
          var l = t & O, d = bo(e), v = d.length, T = bo(n), L = T.length;
          if (v != L && !l)
            return !1;
          for (var W = v; W--; ) {
            var H = d[W];
            if (!(l ? H in n : Te.call(n, H)))
              return !1;
          }
          var j = s.get(e), se = s.get(n);
          if (j && se)
            return j == n && se == e;
          var _e = !0;
          s.set(e, n), s.set(n, e);
          for (var le = l; ++W < v; ) {
            ...
            var we = e[H], Se = n[H];
            if (r)
              var yn = l ? r(Se, we, H, n, e, s) : r(we, Se, H, e, n, s);
...
`);
        ...
        function Nf(e, n) {
          var t = e[1], r = n[1], i = t | r, s = i < (V | $ | ve), l = r == ve && t == J || r == ve && t == Ve && e[7].length <= n[8] || r == (ve | Ve) && n[7].length <= n[8] && t == J;
          if (!(s || l))
            return e;
          ...
          if (d) {
            var v = e[3];
            e[3] = v ? Is(v, d, n[4]) : d, e[4] = v ? ut(e[3], w) : n[4];
          }
          return d = n[5], d && (v = e[5], e[5] = v ? $s(v, d, n[6]) : d, e[6] = v ? ut(e[5], w) : n[6]), d = n[7], d && (e[7] = d), r & ve && (e[8] = e[8] == null ? n[8] : nn(e[8], n[8])), e[9] == null && (e[9] = n[9]), e[0] = n[0], e[1] = i, e;
        }
        function Ff(e) {
          var n = [];
          if (e != null)
        ...
        function cl(e, n, t) {
          n = t ? o : n;
          var r = Qn(e, J, o, o, o, o, o, n);
          return r.placeholder = cl.placeholder, r;
        }
        function fl(e, n, t) {
          n = t ? o : n;
          var r = Qn(e, ue, o, o, o, o, o, n);
          return r.placeholder = fl.placeholder, r;
        }
        function pl(e, n, t) {
          var r, i, s, l, d, v, T = 0, L = !1, W = !1, H = !0;
          if (typeof e != "function")
            throw new Cn(c);
          n = En(n) || 0, Me(t) && (L = !!t.leading, W = "maxWait" in t, s = W ? qe(En(t.maxWait) || 0, n) : s, H = "trailing" in t ? !!t.trailing : H);
          function j(Fe) {
            var Pn = r, rt = i;
            return r = i = o, T = Fe, l = e.apply(rt, Pn), l;
          }
          function se(Fe) {
            return T = Fe, d = pr(we, n), L ? j(Fe) : l;
          }
          function _e(Fe) {
            var Pn = Fe - v, rt = Fe - T, Ol = n - Pn;
            return W ? nn(Ol, s - rt) : Ol;
          }
          function le(Fe) {
            var Pn = Fe - v, rt = Fe - T;
            return v === o || Pn >= n || Pn < 0 || W && rt >= s;
          }
          function we() {
            var Fe = ii();
            if (le(Fe))
          ...
          function Se(Fe) {
            return d = o, H && r ? j(Fe) : (r = i = o, l);
          }
          function yn() {
            d !== o && xs(d), T = 0, r = v = i = d = o;
          }
          function an() {
            return d === o ? l : Se(ii());
          }
        ...
        function Mg(e, n) {
          return n = typeof n == "function" ? n : o, In(e, G, n);
        }
        function Ug(e) {
          return In(e, K | G);
        }
        function Ng(e, n) {
          return n = typeof n == "function" ? n : o, In(e, K | G, n);
        }
        function Fg(e, n) {
        ...
        var hv = me(function(e, n) {
          ...
          for (i && on(n[0], n[1], i) && (r = 1); ++t < r; )
            for (var s = n[t], l = fn(s), d = -1, v = l.length; ++d < v; ) {
              var T = l[d], L = e[T];
              (L === o || Vn(L, Dt[T]) && !Te.call(e, T)) && (e[T] = s[T]);
            }
          return e;
        }), _v = me(function(e) {
        ...
        function Mv(e, n, t) {
          ...
          if (n = oe(n, 4), t == null) {
            ...
            i ? t = r ? new s() : [] : Me(e) ? t = nt(s) ? Pt(Br(e)) : {} : t = {};
          }
        ...
        function Hv(e, n, t) {
          if (t && typeof t != "boolean" && on(e, n, t) && (n = t = o), t === o && (typeof n == "boolean" ? (t = n, n = o) : typeof e == "boolean" && (t = e, e = o)), e === o && n === o ? (e = 0, n = 1) : (e = tt(e), n === o ? (n = e, e = 0) : n = tt(n)), e > n) {
            var r = e;
            e = n, n = r;
          }
          if (t || e % 1 || n % 1) {
        ...
        function dh(e, n, t) {
          ...
          var i = ui({}, n.imports, r.imports, Ps), s = He(i), l = Hi(i, s), d, v, T = 0, L = n.interpolate || kr, W = "__p += '", H = Yi(
...
`;
          e.replace(H, function(le, we, Se, yn, an, wn) {
            return Se || (Se = yn), W += e.slice(T, wn).replace(Zu, Pd), we && (d = !0, W += `' +
__e(` + we + `) +
'`), an && (v = !0, W += `';
` + an + `;
__p += '`), Se && (W += `' +
...
`) + W + `return __p
          ...
          var _e = El(function() {
            return Ie(s, j + "return " + W).apply(o, l);
          });
        ...
        function hh(e, n) {
          ...
          if (l && (d += v.length - d), Lo(i)) {
            if (e.slice(d).search(i)) {
              var T, L = v;
              for (i.global || (i = Yi(i.source, Re(sa.exec(i)) + "g")), i.lastIndex = 0; T = i.exec(L); )
                var W = T.index;
              v = v.slice(0, W === o ? d : W);
            }
          } else if (e.indexOf(mn(i), d) != d) {
            var H = v.lastIndexOf(i);
            H > -1 && (v = v.slice(0, H));
          }
          return v + r;
        }
        ...
        function Po(e) {
          return us(typeof e == "function" ? e : In(e, K));
        }
        function Ch(e) {
          return cs(In(e, K));
        }
        ...
        function Mo(e, n, t) {
          ...
          var s = !(Me(t) && "chain" in t) || !!t.chain, l = nt(e);
          return Sn(i, function(d) {
            ...
            e[d] = v, l && (e.prototype[d] = function() {
              ...
              if (s || T) {
                ...
                return W.push({ func: v, args: arguments, thisArg: e }), L.__chain__ = T, L;
              }
        ...
        return a.after = Cg, a.ary = ll, a.assign = fv, a.assignIn = kl, a.assignInWith = ui, a.assignWith = pv, a.at = gv, a.before = ul, a.bind = Eo, a.bindAll = bh, a.bindKey = dl, a.castArray = Vg, a.chain = ol, a.chunk = Hf, a.compact = Kf, a.concat = Yf, a.cond = yh, a.conforms = wh, a.constant = Vo, a.countBy = tg, a.create = vv, a.curry = cl, a.curryRight = fl, a.debounce = pl, a.defaults = hv, a.defaultsDeep = _v, a.defer = Ag, a.delay = Ig, a.difference = Zf, a.differenceBy = Jf, a.differenceWith = Xf, a.drop = Qf, a.dropRight = jf, a.dropRightWhile = ep, a.dropWhile = np, a.fill = tp, a.filter = ig, a.flatMap = sg, a.flatMapDeep = lg, a.flatMapDepth = ug, a.flatten = nl, a.flattenDeep = rp, a.flattenDepth = ip, a.flip = $g, a.flow = xh, a.flowRight = Sh, a.fromPairs = op, a.functions = Sv, a.functionsIn = Cv, a.groupBy = dg, a.initial = sp, a.intersection = lp, a.intersectionBy = up, a.intersectionWith = dp, a.invert = Iv, a.invertBy = $v, a.invokeMap = fg, a.iteratee = Po, a.keyBy = pg, a.keys = He, a.keysIn = fn, a.map = ri, a.mapKeys = Ev, a.mapValues = Tv, a.matches = Ch, a.matchesProperty = Ah, a.memoize = oi, a.merge = Ov, a.mergeWith = xl, a.method = Ih, a.methodOf = $h, a.mixin = Mo, a.negate = ai, a.nthArg = Eh, a.omit = Lv, a.omitBy = Bv, a.once = Rg, a.orderBy = gg, a.over = Th, a.overArgs = Eg, a.overEvery = Oh, a.overSome = Lh, a.partial = To, a.partialRight = gl, a.partition = vg, a.pick = Dv, a.pickBy = Sl, a.property = Tl, a.propertyOf = Bh, a.pull = gp, a.pullAll = rl, a.pullAllBy = vp, a.pullAllWith = hp, a.pullAt = _p, a.range = Dh, a.rangeRight = Wh, a.rearg = Tg, a.reject = mg, a.remove = mp, a.rest = Og, a.reverse = $o, a.sampleSize = yg, a.set = Vv, a.setWith = Pv, a.shuffle = wg, a.slice = bp, a.sortBy = Sg, a.sortedUniq = Ap, a.sortedUniqBy = Ip, a.split = sh, a.spread = Lg, a.tail = $p, a.take = Rp, a.takeRight = Ep, a.takeRightWhile = Tp, a.takeWhile = Op, a.tap = Kp, a.throttle = Bg, a.thru = ti, a.toArray = bl, a.toPairs = Cl, a.toPairsIn = Al, a.toPath = Nh, a.toPlainObject = wl, a.transform = Mv, a.unary = Dg, a.union = Lp, a.unionBy = Bp, a.unionWith = Dp, a.uniq = Wp, a.uniqBy = Vp, a.uniqWith = Pp, a.unset = Uv, a.unzip = Ro, a.unzipWith = il, a.update = Nv, a.updateWith = Fv, a.values = zt, a.valuesIn = zv, a.without = Mp, a.words = Rl, a.wrap = Wg, a.xor = Up, a.xorBy = Np, a.xorWith = Fp, a.zip = zp, a.zipObject = qp, a.zipObjectDeep = Gp, a.zipWith = Hp, a.entries = Cl, a.entriesIn = Al, a.extend = kl, a.extendWith = ui, Mo(a, a), a.add = zh, a.attempt = El, a.camelCase = Kv, a.capitalize = Il, a.ceil = qh, a.clamp = qv, a.clone = Pg, a.cloneDeep = Ug, a.cloneDeepWith = Ng, a.cloneWith = Mg, a.conformsTo = Fg, a.deburr = $l, a.defaultTo = kh, a.divide = Gh, a.endsWith = Yv, a.eq = Vn, a.escape = Zv, a.escapeRegExp = Jv, a.every = rg, a.find = og, a.findIndex = js, a.findKey = mv, a.findLast = ag, a.findLastIndex = el, a.findLastKey = bv, a.floor = Hh, a.forEach = al, a.forEachRight = sl, a.forIn = yv, a.forInRight = wv, a.forOwn = kv, a.forOwnRight = xv, a.get = Bo, a.gt = zg, a.gte = qg, a.has = Av, a.hasIn = Do, a.head = tl, a.identity = pn, a.includes = cg, a.indexOf = ap, a.inRange = Gv, a.invoke = Rv, a.isArguments = At, a.isArray = ge, a.isArrayBuffer = Gg, a.isArrayLike = cn, a.isArrayLikeObject = Ne, a.isBoolean = Hg, a.isBuffer = vt, a.isDate = Kg, a.isElement = Yg, a.isEmpty = Zg, a.isEqual = Jg, a.isEqualWith = Xg, a.isError = Oo, a.isFinite = Qg, a.isFunction = nt, a.isInteger = vl, a.isLength = si, a.isMap = hl, a.isMatch = jg, a.isMatchWith = ev, a.isNaN = nv, a.isNative = tv, a.isNil = iv, a.isNull = rv, a.isNumber = _l, a.isObject = Me, a.isObjectLike = Ue, a.isPlainObject = gr, a.isRegExp = Lo, a.isSafeInteger = ov, a.isSet = ml, a.isString = li, a.isSymbol = bn, a.isTypedArray = Ft, a.isUndefined = av, a.isWeakMap = sv, a.isWeakSet = lv, a.join = cp, a.kebabCase = Xv, a.last = Rn, a.lastIndexOf = fp, a.lowerCase = Qv, a.lowerFirst = jv, a.lt = uv, a.lte = dv, a.max = Kh, a.maxBy = Yh, a.mean = Zh, a.meanBy = Jh, a.min = Xh, a.minBy = Qh, a.stubArray = No, a.stubFalse = Fo, a.stubObject = Vh, a.stubString = Ph, a.stubTrue = Mh, a.multiply = jh, a.nth = pp, a.noConflict = Rh, a.noop = Uo, a.now = ii, a.pad = eh, a.padEnd = nh, a.padStart = th, a.parseInt = rh, a.random = Hv, a.reduce = hg, a.reduceRight = _g, a.repeat = ih, a.replace = oh, a.result = Wv, a.round = e_, a.runInContext = p, a.sample = bg, a.size = kg, a.snakeCase = ah, a.some = xg, a.sortedIndex = yp, a.sortedIndexBy = wp, a.sortedIndexOf = kp, a.sortedLastIndex = xp, a.sortedLastIndexBy = Sp, a.sortedLastIndexOf = Cp, a.startCase = lh, a.startsWith = uh, a.subtract = n_, a.sum = t_, a.sumBy = r_, a.template = dh, a.times = Uh, a.toFinite = tt, a.toInteger = he, a.toLength = yl, a.toLower = ch, a.toNumber = En, a.toSafeInteger = cv, a.toString = Re, a.toUpper = fh, a.trim = ph, a.trimEnd = gh, a.trimStart = vh, a.truncate = hh, a.unescape = _h, a.uniqueId = Fh, a.upperCase = mh, a.upperFirst = Wo, a.each = al, a.eachRight = sl, a.first = tl, Mo(a, (function() {
          ...
          i && (a.prototype[n] = function() {
            var l = this.__wrapped__, d = r ? [1] : arguments, v = l instanceof ke, T = d[0], L = v || ge(l), W = function(we) {
              var Se = i.apply(a, lt([we], d));
              return r && H ? Se[0] : Se;
            };
            L && t && typeof T == "function" && T.length != 1 && (v = L = !1);
            var H = this.__chain__, j = !!this.__actions__.length, se = s && !H, _e = v && !j;
            if (!s && L) {
              l = _e ? l : new ke(this);
              var le = e.apply(l, d);
              return le.__actions__.push({ func: ti, args: [W], thisArg: o }), new An(le, H);
            }
...
var N0 = U0();
const F0 = { class: "editor" }, z0 = { class: "editor__fields" }, q0 = { class: "editor__actions" }, G0 = {
  key: 1,
  ...
  emits: ["close"],
  setup(g, { emit: h }) {
    const o = g, y = ee({}), m = xe(ta), I = xe(at), c = Tn(I, (O) => O.connections), E = Tn(I, (O) => O.datasources), C = ne(() => m.registeredDatasources);
    Hn(() => {
      const O = m.getDatasourceModel(o.itemId);
      y.value = O ? {
        uid: O.uid,
        name: O.name,
        type: O.type,
        config: N0.cloneDeep(O.config ?? {})
      } : {};
    });
    const f = () => {
      const O = m.getDatasourceModel(o.itemId);
      O && (O.name = y.value.name, O.type = y.value.type, O.config = y.value.config, O.connection = c.value.find(
        (Y) => Y.uid === y.value.config?.connection
      ), m.saveDatasource(O)), G("close");
    }, w = ne(() => {
      const O = m.getDatasourceIdentifiers(y.value.type);
      return O ? m.resolveIdentifier(O.Preview) : null;
    }), K = ne(() => {
      const O = m.getDatasourceIdentifiers(y.value.type);
      return O ? m.resolveIdentifier(O.Settings) : null;
    }), q = (O) => {
      y.value.config = O;
    }, G = h;
    return (O, Y) => (_(), A("div", F0, [
      g.view === "settings" ? (_(), A(Ce, { key: 0 }, [
        u("div", z0, [
          B(b(gn), {
            modelValue: y.value.uid,
            "onUpdate:modelValue": Y[0] || (Y[0] = (V) => y.value.uid = V),
            label: "UID",
            readonly: ""
          }, null, 8, ["modelValue"]),
          B(b(gn), {
            modelValue: y.value.name,
            "onUpdate:modelValue": Y[1] || (Y[1] = (V) => y.value.name = V),
            label: "Name"
          }, null, 8, ["modelValue"]),
          B(b(Un), {
            modelValue: y.value.type,
            "onUpdate:modelValue": Y[2] || (Y[2] = (V) => y.value.type = V),
            label: "Typ",
            options: C.value
        ...
        u("div", q0, [
          B(b(be), {
            intent: "quiet",
            onClick: Y[3] || (Y[3] = (V) => O.$emit("close"))
          }, {
            default: te(() => [...Y[4] || (Y[4] = [
              ae("Schließen", -1)
            ])]),
...
}), K0 = /* @__PURE__ */ je(H0, [["__scopeId", "data-v-b30445c3"]]), Y0 = { class: "editor" }, Z0 = { class: "editor__fields" }, J0 = { class: "editor__actions" }, X0 = /* @__PURE__ */ Qe({
  ...
  emits: ["close"],
  setup(g, { emit: h }) {
    const o = g, y = h, m = ee({}), I = xe(na), c = ne(() => I.registeredConnections);
    Hn(() => {
    ...
    const E = ne(() => {
      const f = I.getConnectionIdentifiers(m.value.type);
      return f ? I.resolveIdentifier(f.Settings) : null;
    }), C = () => {
      const f = I.getConnectionModel(o.itemId);
      f && (f.name = m.value.name, f.type = m.value.type, f.config = m.value.config, I.saveConnection(f)), y("close");
    };
    return (f, w) => (_(), A("div", Y0, [
      u("div", Z0, [
        B(b(gn), {
          modelValue: m.value.uid,
          "onUpdate:modelValue": w[0] || (w[0] = (K) => m.value.uid = K),
          label: "UID",
          readonly: ""
        }, null, 8, ["modelValue"]),
        B(b(gn), {
          modelValue: m.value.name,
          "onUpdate:modelValue": w[1] || (w[1] = (K) => m.value.name = K),
          label: "Name"
        }, null, 8, ["modelValue"]),
        B(b(Un), {
          modelValue: m.value.type,
          "onUpdate:modelValue": w[2] || (w[2] = (K) => m.value.type = K),
          label: "Typ",
          options: c.value
        }, null, 8, ["modelValue", "options"]),
        (_(), Be(Kn(E.value), {
      ...
      u("div", J0, [
        B(b(be), {
          intent: "quiet",
          onClick: w[3] || (w[3] = (K) => f.$emit("close"))
        }, {
          default: te(() => [...w[4] || (w[4] = [
            ae("Schließen", -1)
          ])]),
...
}), Q0 = /* @__PURE__ */ je(X0, [["__scopeId", "data-v-c18a8ecf"]]), j0 = { class: "data-page" }, eb = { class: "data-page__tree" }, nb = { class: "data-page__detail" }, tb = {
  key: 0,
  class: "data-page__nothing"
}, rb = { class: "detail__head" }, ib = { class: "detail__what" }, ob = { class: "detail__name" }, ab = {
  key: 0,
  class: "detail__sub"
}, sb = {
  key: 0,
  class: "detail__usage"
}, lb = { class: "detail__body" }, ub = /* @__PURE__ */ Qe({
  __name: "ConnectionsAndData",
  setup(g) {
    const h = xe(at), o = Tn(h, (O) => O.connections), y = Tn(h, (O) => O.datasources), { usageOf: m, usageLabel: I } = yu(), c = ee(void 0), E = ee("preview"), C = xe("endpointfinder", null), f = () => C?.(), w = Yl(() => {
      const O = c.value;
      if (O)
        return O.type === "Connection" ? o.value.find((Y) => Y.uid === O.itemId) : y.value.find((Y) => Y.uid === O.itemId);
    }), K = ne(() => {
      const O = w.value;
      return O ? c.value?.type === "Connection" ? O.type ?? "" : [O.type, O.connection?.name].filter(Boolean).join(" · ") : "";
    }), q = ne(() => c.value?.type !== "DataSource" || !c.value.itemId ? "" : I(m(c.value.itemId))), G = ne(
    ...
    return Nn(c, (O) => {
      E.value = O?.type === "DataSource" ? "preview" : "settings";
    }), (O, Y) => (_(), A("div", j0, [
      u("aside", eb, [
        B(P0, {
          modelValue: c.value,
      ...
      u("section", nb, [
        c.value ? (_(), A(Ce, { key: 1 }, [
          u("header", rb, [
            u("div", ib, [
              u("h1", ob, z(b(w)?.name ?? c.value.itemId), 1),
              K.value ? (_(), A("span", ab, z(K.value), 1)) : re("", !0)
            ]),
            q.value ? (_(), A("span", sb, "Verwendet in " + z(q.value), 1)) : re("", !0)
          ]),
          B(b(w_), {
            modelValue: E.value,
          ...
          u("div", lb, [
            c.value.type === "Connection" ? (_(), Be(Q0, {
              key: c.value.itemId,
              "item-id": c.value.itemId,
              onClose: Y[2] || (Y[2] = (V) => c.value = void 0)
            }, null, 8, ["item-id"])) : (_(), Be(K0, {
              key: `${c.value.itemId}-${E.value}`,
              "item-id": c.value.itemId,
              view: E.value === "preview" ? "preview" : "settings",
              onClose: Y[3] || (Y[3] = (V) => c.value = void 0)
            }, null, 8, ["item-id", "view"]))
          ])
        ], 64)) : (_(), A("p", tb, " Wähle links eine Verbindung oder eine Datenquelle. "))
      ])
    ]));
  }
}), db = /* @__PURE__ */ je(ub, [["__scopeId", "data-v-91f5d575"]]), cb = { class: "add_widget_window" }, fb = { class: "add_widget_window__scroll" }, pb = { class: "widgets_grid-icon" }, gb = ["src"], vb = { class: "widgets_grid-name" }, hb = /* @__PURE__ */ Qe({
  __name: "AddWidgetWindow",
  setup(g) {
    const h = ee(""), o = ee(""), y = Tn(xe(at), (f) => f.datasources), m = (f) => {
    ...
    const c = Object.entries(I.getAllWidgets()).map(([f, w]) => ({ type: f, name: w.name, icon: w.icon })), E = ne(() => c);
    ne(() => [
      "None",
      ...y.value.map((f) => f.type).filter((f, w, K) => w === K.indexOf(f))
    ]);
    const C = ne(() => y.value.filter((f) => f.type === o.value).map((f) => ({ uid: f.uid })));
    return Nn(o, (f) => {
      !f || f === "None" ? h.value = "" : C.value.map((K) => K.uid).includes(h.value) || (h.value = "");
    }), (f, w) => (_(), A("div", cb, [
      u("div", fb, [
        B(b(U_), {
          class: "widgets_grid",
...
}), _b = /* @__PURE__ */ je(hb, [["__scopeId", "data-v-3ac9fb06"]]), mb = {
  class: "overlay",
  role: "dialog",
  "aria-modal": "true",
  "aria-label": "Widget-Einstellungen"
}, bb = { class: "stage" }, yb = { class: "stage__head" }, wb = { class: "stage__name" }, kb = { class: "stage__uid" }, xb = {
  class: "seg",
  role: "group",
  "aria-label": "Vorschaugröße"
}, Sb = { class: "stage__bar" }, Cb = { class: "stage__body" }, Ab = {
  class: "tabs",
  role: "tablist",
  "aria-label": "Einstellungen"
}, Ib = ["aria-selected"], $b = { class: "tab__n" }, Rb = ["aria-selected"], Eb = ["aria-selected", "onClick"], Tb = ["aria-selected"], Ob = ["aria-selected"], Lb = { class: "tab__n" }, Bb = ["aria-selected"], Db = { class: "tab__n" }, Wb = {
  key: 0,
  class: "fields"
}, Vb = {
  key: 0,
  class: "bound"
}, Pb = { class: "bound__name" }, Mb = { class: "bound__var" }, Ub = {
  key: 1,
  class: "note"
}, Nb = { class: "foot" }, Fb = ":scope > [data-section], :scope > .va-collapse", zb = /* @__PURE__ */ Qe({
  ...
  setup(g, { emit: h }) {
    const o = h, y = jo(g, "modelValue"), m = g, I = Tn(xe(at), (k) => k.datasources), E = xe(nu).getAllWidgets();
    xe("i18n");
    const C = ee("look"), f = ne(() => {
      const k = y.value?.type;
      return k ? E[k]?.settingsForm : void 0;
    }), w = ne(
      () => !!(y.value?.type && E[y.value.type]?.settingsComponent)
    ), K = ne(() => !!f.value && w.value), q = ne(() => !!f.value || w.value);
    Nn(q, (k) => {
      !k && C.value === "look" && (C.value = "data");
    }, { immediate: !0 });
    const G = ee(), O = ee([]), Y = ee(0);
    function V(k) {
      return k ? [...k.querySelectorAll(Fb)] : [];
    }
    function $() {
      return V(G.value);
    }
    function D(k) {
      ...
      if (x) return x;
      const Z = k.querySelector(".va-collapse__header-wrapper");
      return Z ? [...Z.querySelectorAll("*")].filter((Ae) => !Ae.classList.contains("va-icon") && Ae.children.length === 0).map((Ae) => (Ae.textContent ?? "").trim()).filter((Ae) => Ae && !/^(expand_more|expand_less|add_circle)$/.test(Ae))[0] ?? "" : "";
    }
    function J(k) {
      return k.className.includes("--expanded");
    }
    function ue(k) {
      Y.value = k, $().forEach((x, Z) => {
        const Oe = Z === k;
        if (x.classList.contains("va-collapse")) {
          const Ae = x.querySelector(".va-collapse__header-wrapper");
          Ae && (Ae.style.display = "none"), Oe !== J(x) && Ae?.click();
        }
    ...
    async function P() {
      await gi();
      const k = $();
      O.value = k.map((x, Z) => ({ label: D(x) || `Abschnitt ${Z + 1}`, index: Z })), k.length && ue(Math.min(Y.value, k.length - 1));
    }
    function Q(k) {
      C.value = "look", ue(k);
    }
    const ve = ee();
    async function Ve() {
      const k = f.value?.unmodelledSections, x = ve.value;
      if (!(!k || !x)) {
        await gi();
        for (const Z of V(x))
          Z.style.display = k.includes(D(Z)) ? "" : "none";
      }
    }
    Nn(C, (k) => {
      k === "rest" && Ve();
    ...
    function en() {
      for (const k of ["wrapperConfig", "config"]) {
        const x = ce(k);
        if (x)
          for (const [Z, Oe] of Object.entries(ie[k]))
            try {
              if (Oe.list) {
                const vn = x[Z];
              ...
              const Ae = x[Z], kn = !!Ae && typeof Ae == "object" && "value" in Ae;
              Oe.hasValue && kn ? Ae.value = Oe.value : x[Z] = Oe.field;
            } catch {
            }
      }
    }
    ...
    const N = ee(!1), M = ne(() => N.value || !m.boardSize ? { width: "100%", height: "100%" } : {
    ...
    function ht(k) {
      ln = !0, k.target.setPointerCapture?.(k.pointerId);
    }
    function $t(k) {
      if (!ln) return;
    ...
    function On(k) {
      sn.value = Math.min(Math.max(sn.value + k, 320), window.innerWidth - 360), _t();
    }
    const Fn = ne(() => {
      const k = [], x = [
        ["Rahmen", y.value?.wrapperConfig],
        ["Darstellung", y.value?.config]
      ];
      for (const [Z, Oe] of x)
        for (const [Ae, kn] of Object.entries(Oe ?? {})) {
          const vn = kn?.variable;
          vn && k.push({ group: Z, name: Ae, variable: String(vn?.name ?? vn) });
        }
      return k;
    }), Zt = ne(() => Object.keys(y.value?.wrapperConfig ?? {}).length), Yn = ne(() => y.value?.config?.datasourceId ? 1 : 0);
    function U(k) {
      k.key === "Escape" && ye();
    }
    return Nn(C, (k) => {
      ...
      u("div", {
        class: "scrim",
        onClick: x[14] || (x[14] = Mn((Z) => ye(), ["self"]))
      }, [
        u("section", mb, [
          u("div", bb, [
            u("header", yb, [
              u("span", wb, z(y.value?.type ?? "Widget"), 1),
              u("code", kb, z(y.value?.uid), 1),
              x[15] || (x[15] = u("span", { class: "stage__spacer" }, null, -1)),
              u("div", xb, [
                u("button", {
                  type: "button",
                  class: Ee({ on: !N.value }),
                  onClick: x[0] || (x[0] = (Z) => N.value = !1)
                }, "Boardgröße", 2),
                u("button", {
                  type: "button",
                  class: Ee({ on: N.value }),
                  onClick: x[1] || (x[1] = (Z) => N.value = !0)
                }, "Füllen", 2)
              ])
            ]),
            u("div", Sb, z(N.value ? "Auf die Fläche gestreckt" : pe.value), 1),
            u("div", Cb, [
              u("div", {
                class: "preview",
                style: Yt(M.value)
          ...
          u("div", {
            ...
            onKeydown: [
              x[2] || (x[2] = vi(Mn((Z) => On(16), ["prevent"]), ["left"])),
              x[3] || (x[3] = vi(Mn((Z) => On(-16), ["prevent"]), ["right"]))
            ]
          }, null, 32),
          u("div", {
            class: "side",
            ...
            u("nav", Ab, [
              u("button", {
                ...
                onClick: x[4] || (x[4] = (Z) => C.value = "data")
              }, [
                x[16] || (x[16] = ae(" Daten ", -1)),
                u("span", $b, z(Yn.value), 1)
              ], 10, Ib),
              !O.value.length && q.value ? (_(), A("button", {
                key: 0,
                type: "button",
                role: "tab",
                "aria-selected": C.value === "look",
                class: Ee(["tab", { on: C.value === "look" }]),
                onClick: x[5] || (x[5] = (Z) => C.value = "look")
              }, " Darstellung ", 10, Rb)) : re("", !0),
              (_(!0), A(Ce, null, Ge(O.value, (Z) => (_(), A("button", {
                key: Z.index,
                type: "button",
                role: "tab",
                "aria-selected": C.value === "look" && Y.value === Z.index,
                class: Ee(["tab", { on: C.value === "look" && Y.value === Z.index }]),
                onClick: (Oe) => Q(Z.index)
              }, z(Z.label), 11, Eb))), 128)),
              K.value ? (_(), A("button", {
                key: 1,
                type: "button",
                ...
                onClick: x[6] || (x[6] = (Z) => C.value = "rest")
              }, " Weiteres ", 10, Tb)) : re("", !0),
              u("button", {
                type: "button",
                role: "tab",
                ...
                onClick: x[7] || (x[7] = (Z) => C.value = "frame")
              }, [
                x[17] || (x[17] = ae(" Rahmen ", -1)),
                u("span", Lb, z(Zt.value), 1)
              ], 10, Ob),
              u("button", {
                type: "button",
                role: "tab",
                ...
                onClick: x[8] || (x[8] = (Z) => C.value = "variables")
              }, [
                x[18] || (x[18] = ae(" Variablen ", -1)),
                u("span", Db, z(Fn.value.length), 1)
              ], 10, Bb)
            ]),
            y.value ? (_(), A("div", Wb, [
              C.value === "data" ? (_(), A(Ce, { key: 0 }, [
                B(b(Un), {
                  modelValue: y.value.config.datasourceId,
                  "onUpdate:modelValue": x[9] || (x[9] = (Z) => y.value.config.datasourceId = Z),
                  label: "Datenquelle",
                  class: "pick",
                  options: b(I),
                  clearable: ""
              ...
              Kt(u("div", {
                ...
                f.value ? (_(), Be(b(_i), {
                  key: 0,
                  modelValue: y.value.config,
                  "onUpdate:modelValue": x[10] || (x[10] = (Z) => y.value.config = Z),
                ...
                }, null, 8, ["modelValue", "create", "ui-model-xmi", "domain-package", "ui-model-uri", "entry-forms"])) : (_(), Be(Kn(b(E)[y.value.type ?? ""]?.settingsComponent), {
                  modelValue: y.value.config,
                  "onUpdate:modelValue": x[11] || (x[11] = (Z) => y.value.config = Z),
                  key: y.value.uid,
                  dataSources: b(I)
              ...
              Kt(u("div", {
                ...
                K.value ? (_(), Be(Kn(b(E)[y.value.type ?? ""]?.settingsComponent), {
                  modelValue: y.value.config,
                  "onUpdate:modelValue": x[12] || (x[12] = (Z) => y.value.config = Z),
                  key: y.value.uid + "-rest",
                  dataSources: b(I)
              ...
              Kt(u("div", null, [
                B(b(_i), {
                  modelValue: y.value.wrapperConfig,
                  "onUpdate:modelValue": x[13] || (x[13] = (Z) => y.value.wrapperConfig = Z),
                  create: () => new (b(z_))(),
                  "ui-model-xmi": b(F_),
                  "domain-package": b(N_).eINSTANCE,
              ...
              C.value === "variables" ? (_(), A(Ce, { key: 1 }, [
                Fn.value.length ? (_(), A("table", Vb, [
                  ...
                  u("tbody", null, [
                    (_(!0), A(Ce, null, Ge(Fn.value, (Z) => (_(), A("tr", {
                      key: Z.group + Z.name
                    }, [
                      u("td", Pb, z(Z.name), 1),
                      u("td", null, z(Z.group), 1),
                      u("td", Mb, z(Z.variable), 1)
                    ]))), 128))
                  ])
                ])) : (_(), A("p", Ub, [...x[22] || (x[22] = [
...
}), qb = /* @__PURE__ */ je(zb, [["__scopeId", "data-v-67bd2420"]]), Gb = {
  class: "page-settings",
  "aria-label": "Seite einrichten"
}, Hb = { class: "head" }, Kb = { class: "body" }, Yb = {
  key: 0,
  class: "missing"
}, Zb = { class: "group" }, Jb = { class: "group" }, Xb = { class: "group" }, Qb = { class: "group" }, jb = { class: "ident" }, ey = { class: "foot" }, ny = /* @__PURE__ */ Qe({
  __name: "PageSettings",
  props: {
  ...
  emits: /* @__PURE__ */ hi(["close"], ["update:modelValue"]),
  setup(g, { emit: h }) {
    const o = jo(g, "modelValue"), y = h, m = xe(yr), I = xe(yi), c = ee(null), E = ne(() => I?.getAllLayouts() ?? []), C = ne(
      () => E.value.find(
    ...
    ], O = ne(() => !!c.value?.backgroundImage?.trim());
    function Y() {
      if (!o.value || !m) {
        c.value = null;
        return;
    ...
    return Hn(Y), Nn(o, Y), Nn(
      ...
      u("header", Hb, [
        $[14] || ($[14] = u("h2", { class: "head__title" }, "Seite einrichten", -1)),
        u("button", {
          type: "button",
          class: "head__close",
          "aria-label": "Schließen",
          onClick: $[0] || ($[0] = (D) => y("close"))
        }, " × ")
      ]),
      u("div", Kb, [
        c.value ? (_(), A(Ce, { key: 1 }, [
          u("section", Zb, [
            $[15] || ($[15] = u("h3", { class: "group__label" }, "Seite", -1)),
            B(b(gn), {
              modelValue: c.value.name,
              "onUpdate:modelValue": $[1] || ($[1] = (D) => c.value.name = D),
              label: "Name"
            }, null, 8, ["modelValue"]),
            B(b(gn), {
              modelValue: c.value.description,
              "onUpdate:modelValue": $[2] || ($[2] = (D) => c.value.description = D),
              label: "Beschreibung"
            }, null, 8, ["modelValue"]),
            B(b(gn), {
              modelValue: c.value.icon,
              "onUpdate:modelValue": $[3] || ($[3] = (D) => c.value.icon = D),
              label: "Symbol",
              placeholder: "Name eines Material-Icons"
            }, null, 8, ["modelValue"])
          ]),
          u("section", Jb, [
            $[16] || ($[16] = u("h3", { class: "group__label" }, "Layout", -1)),
            B(b(Un), {
              modelValue: f.value,
              "onUpdate:modelValue": $[4] || ($[4] = (D) => f.value = D),
              label: "Layout",
              options: E.value,
              "value-key": "id",
              "label-key": "name"
            }, null, 8, ["modelValue", "options"]),
            w.value?.settingsForm ? (_(), Be(b(_i), {
              key: 0,
              modelValue: c.value.layoutSettings,
              "onUpdate:modelValue": $[5] || ($[5] = (D) => c.value.layoutSettings = D),
            ...
            }, null, 8, ["modelValue", "create", "ui-model-xmi", "domain-package", "ui-model-uri", "entry-forms"])) : w.value?.settings ? (_(), Be(Kn(w.value.settings), {
              key: 1,
              modelValue: c.value.layoutSettings,
              "onUpdate:modelValue": $[6] || ($[6] = (D) => c.value.layoutSettings = D)
            }, null, 8, ["modelValue"])) : re("", !0)
          ]),
          u("section", Xb, [
            $[17] || ($[17] = u("h3", { class: "group__label" }, "Hintergrund", -1)),
            B(b(Zl), {
              modelValue: c.value.backgroundColor,
              "onUpdate:modelValue": $[7] || ($[7] = (D) => c.value.backgroundColor = D),
              label: "Farbe",
              stacked: ""
            }, null, 8, ["modelValue"]),
            B(b(gn), {
              modelValue: c.value.backgroundImage,
              "onUpdate:modelValue": $[8] || ($[8] = (D) => c.value.backgroundImage = D),
              label: "Bild",
              placeholder: "Adresse eines Bildes"
            }, null, 8, ["modelValue"]),
            O.value ? (_(), A(Ce, { key: 0 }, [
              B(b(Un), {
                modelValue: c.value.backgroundSize,
                "onUpdate:modelValue": $[9] || ($[9] = (D) => c.value.backgroundSize = D),
                label: "Größe",
                options: K
              }, null, 8, ["modelValue"]),
              B(b(Un), {
                modelValue: c.value.backgroundRepeat,
                "onUpdate:modelValue": $[10] || ($[10] = (D) => c.value.backgroundRepeat = D),
                label: "Wiederholung",
                options: q
              }, null, 8, ["modelValue"]),
              B(b(Un), {
                modelValue: c.value.backgroundPosition,
                "onUpdate:modelValue": $[11] || ($[11] = (D) => c.value.backgroundPosition = D),
                label: "Position",
                options: G
              }, null, 8, ["modelValue"])
            ], 64)) : re("", !0)
          ...
          u("section", Qb, [
            $[18] || ($[18] = u("h3", { class: "group__label" }, "Navigation", -1)),
            B(b(Jl), {
              modelValue: c.value.visibleInNavigation,
              "onUpdate:modelValue": $[12] || ($[12] = (D) => c.value.visibleInNavigation = D),
        ...
        ], 64)) : (_(), A("p", Yb, " Diese Seite ist nicht mehr da. Wähle oben eine andere. "))
      ]),
      u("footer", ey, [
        B(b(be), {
          intent: "primary",
          onClick: $[13] || ($[13] = (D) => y("close"))
        }, {
          default: te(() => [...$[20] || ($[20] = [
            ae("Fertig", -1)
          ])]),
...
}), ty = /* @__PURE__ */ je(ny, [["__scopeId", "data-v-9ea0fdd8"]]), ry = {
  ref: "board",
  class: "editor"
}, iy = { class: "report-container dottet" }, oy = /* @__PURE__ */ Qe({
  __name: "EditReport",
  setup(g) {
    const h = ee(""), y = br().params.pageid ?? "", m = m_(y || ""), I = m.widgets, c = ee([]), E = xe("endpointfinder", null), C = () => {
      E();
    }, f = ne(() => !!E), { settingsFor: w, closeSettings: K } = iu(), q = ($) => {
      h.value = $;
    }, G = ne(() => {
      const $ = I.value.find((D) => D.uid === h.value);
      return $ || c.value.find((D) => D.uid === h.value);
    }), O = ne(() => {
      const $ = m.layout.value.find((D) => D.id === h.value);
      if (!(!$?.width || !$?.height))
    ...
    return ($, D) => (_(), A("div", ry, [
        ...
        B(c_, { duration: 150 }, {
          default: te(() => [
            b(w) ? (_(), Be(ty, {
              key: 0,
              modelValue: b(w),
              "onUpdate:modelValue": D[2] || (D[2] = (J) => f_(w) ? w.value = J : null),
              onClose: b(K)
      ...
      h.value && G.value ? (_(), Be(qb, {
        key: 1,
        modelValue: G.value,
        "onUpdate:modelValue": D[3] || (D[3] = (J) => G.value = J),
        "board-size": O.value,
        onClose: D[4] || (D[4] = (J) => h.value = "")
      }, {
        preview: te(() => [
          B(b(q_), {
            widget: G.value,
...
}), zl = /* @__PURE__ */ je(oy, [["__scopeId", "data-v-bd444b0c"]]), Gt = I_({
  history: $_("/"),
  routes: [
...
}), ay = { class: "variables" }, sy = { class: "variables__head" }, ly = { class: "reach__title" }, uy = { class: "reach__lead" }, dy = {
  key: 0,
  class: "rows"
}, cy = { class: "row__name" }, fy = { class: "row__type" }, py = { class: "row__value" }, gy = { class: "row__tools" }, vy = {
  key: 1,
  class: "reach__empty"
}, hy = { class: "form" }, _y = { class: "confirm__text" }, my = /* @__PURE__ */ Qe({
  __name: "Configuration",
  setup(g) {
    const h = xe(eu), o = xe(at), y = xe(Symbol.for(ql)), m = Tn(o, (ye) => ye.variables), I = Tn(o, (ye) => ye.pages), c = ee(0), E = () => c.value += 1;
    Hn(() => y?.on(Wl.VariableUpdated, E)), Qo(() => y?.off(Wl.VariableUpdated, E));
    function C(ye) {
      c.value;
      try {
    ...
    const f = ne(() => {
      const ye = m.value.filter((M) => (M.scope ?? "global") === "global"), F = I.value.map((M) => ({
        title: M.name,
        lead: "Nur auf diesem Board",
        page: M,
        rows: m.value.filter((pe) => pe.scope === "page" && pe.page === M)
      })).filter((M) => M.rows.length > 0), N = m.value.filter((M) => M.scope === "page" && !M.page);
      return [
    ...
    Hn(() => w.value = h.getRegisteredVariableTypes());
    const K = ne(() => [
      { text: "Überall", value: "" },
    ...
    ], G = ee(!1), O = ee(null), Y = ee(""), V = ee(""), $ = ee(""), D = ee("external-writable"), J = ee({}), ue = ne(() => O.value !== null), P = ne(
      () => Y.value ? h.getVariableIdentifiers(Y.value)?.settingsForm : void 0
    ), Q = ne(
      () => Y.value ? h.getVariableIdentifiers(Y.value)?.Settings : null
    );
    function ve() {
      O.value = null, Y.value = w.value[0] ?? "", V.value = "", $.value = "Variable " + Math.random().toString(36).substring(7), D.value = "external-writable", J.value = {}, G.value = !0;
    }
    function Ve(ye) {
      O.value = ye.uid, Y.value = ye.type ?? "", V.value = ye.page?.id ?? "", $.value = ye.name, D.value = ye.accessMode ?? "external-writable";
    ...
    return (ye, F) => (_(), A("div", ay, [
      ...
      (_(!0), A(Ce, null, Ge(f.value, (N) => (_(), A("section", {
        ...
        u("h2", ly, [
          ae(z(N.title), 1),
          u("span", uy, z(N.lead), 1)
        ]),
        N.rows.length ? (_(), A("ul", dy, [
          (_(!0), A(Ce, null, Ge(N.rows, (M) => (_(), A("li", {
            key: M.uid,
            class: "row"
          }, [
            u("span", cy, z(M.name), 1),
            u("span", fy, z(M.type), 1),
            u("span", py, z(C(M)), 1),
            u("span", gy, [
              B(b(be), {
                intent: "quiet",
      ...
      B(b(Ko), {
        modelValue: G.value,
        "onUpdate:modelValue": F[7] || (F[7] = (N) => G.value = N),
        ...
        actions: te(() => [
          B(b(be), {
            intent: "quiet",
            onClick: F[6] || (F[6] = (N) => G.value = !1)
          }, {
            default: te(() => [...F[12] || (F[12] = [
              ae("Abbrechen", -1)
            ])]),
            _: 1
          }),
        ...
        default: te(() => [
          u("div", hy, [
            ...
            B(b(gn), {
              modelValue: $.value,
              "onUpdate:modelValue": F[3] || (F[3] = (N) => $.value = N),
              label: "Name"
            }, null, 8, ["modelValue"]),
            P.value ? (_(), Be(b(_i), {
              key: Y.value,
              modelValue: J.value,
              "onUpdate:modelValue": F[4] || (F[4] = (N) => J.value = N),
              create: P.value.create,
              "ui-model-xmi": P.value.xmi,
              "domain-package": P.value.ePackage(),
              "ui-model-uri": P.value.uri
            }, null, 8, ["modelValue", "create", "ui-model-xmi", "domain-package", "ui-model-uri"])) : Q.value ? (_(), Be(Kn(Q.value), {
              key: 1,
              modelValue: J.value,
              "onUpdate:modelValue": F[5] || (F[5] = (N) => J.value = N)
            }, null, 8, ["modelValue"])) : re("", !0)
          ])
        ]),
        _: 1
      ...
      B(b(Ko), {
        ...
        default: te(() => [
          u("p", _y, z($e.value?.name) + " wird entfernt. Widgets, die darauf zeigen, finden sie danach nicht mehr. ", 1)
        ]),
        _: 1
      }, 8, ["model-value"])
    ]));
  }
}), by = /* @__PURE__ */ je(my, [["__scopeId", "data-v-9292252d"]]), yy = { class: "appearance" }, wy = { class: "panel" }, ky = { class: "panel__head" }, xy = { class: "panel__tools" }, Sy = {
  key: 0,
  class: "changed"
}, Cy = {
  key: 0,
  class: "gallery"
}, Ay = { class: "demo" }, Iy = { class: "demo__row" }, $y = { class: "demo__row" }, Ry = { class: "demo" }, Ey = { class: "demo__form" }, Ty = { class: "demo" }, Oy = { class: "demo__row" }, Ly = { class: "demo__row" }, By = {
  key: 1,
  class: "body"
}, Dy = {
  class: "themes",
  ...
  class: "theme__strip",
  "aria-hidden": "true"
}, Py = { class: "theme__name" }, My = { class: "theme__note" }, Uy = { class: "tokens" }, Ny = ["aria-expanded", "onClick"], Fy = { class: "group__twist" }, zy = { class: "group__label" }, qy = { class: "group__count" }, Gy = {
  key: 0,
  class: "group__body"
}, Hy = {
  key: 0,
  ...
  "aria-hidden": "true"
}, Yy = { class: "token__text" }, Zy = { class: "token__name" }, Jy = { class: "token__role" }, Xy = ["value", "aria-label", "onInput"], Qy = ["value", "aria-label", "onChange"], jy = ["disabled", "title", "onClick"], ew = /* @__PURE__ */ Qe({
  __name: "Appearance",
  setup(g) {
    const {
      themes: h,
      ...
      clearAllTokens: f,
      exportTheme: w
    } = pu(), K = ee(mi[0].id), q = ee(!1), G = ee("tokens"), O = ee({
      ...
      { uid: "rest", name: "REST" }
    ], V = ne(() => Object.keys(y.value).length);
    function $(ue) {
      return [
        ue.tokens["color-bg"],
    ...
    async function J() {
      try {
        await navigator.clipboard.writeText(JSON.stringify(w(), null, 2)), q.value = !0, setTimeout(() => q.value = !1, 2e3);
      } catch {
        q.value = !1;
      }
    }
    return (ue, P) => (_(), A("div", yy, [
      u("div", wy, [
        u("header", ky, [
          u("button", {
            type: "button",
            class: Ee(["head__tab", { on: G.value === "tokens" }]),
            onClick: P[0] || (P[0] = (Q) => G.value = "tokens")
          }, " Tokens ", 2),
          u("button", {
            type: "button",
            class: Ee(["head__tab", { on: G.value === "controls" }]),
            onClick: P[1] || (P[1] = (Q) => G.value = "controls")
          }, " Elemente ", 2),
          u("span", xy, [
            V.value ? (_(), A("span", Sy, z(V.value) + " " + z(V.value === 1 ? "Wert geändert" : "Werte geändert"), 1)) : re("", !0),
            V.value ? (_(), A("button", {
              key: 1,
              class: "btn",
              type: "button",
            ...
            u("button", {
              ...
              type: "button",
              onClick: J
            }, z(q.value ? "Kopiert" : "Als JSON kopieren"), 1)
          ])
        ]),
        G.value === "controls" ? (_(), A("div", Cy, [
          ...
          u("section", Ay, [
            ...
            u("div", Iy, [
              B(b(be), null, {
                default: te(() => [...P[13] || (P[13] = [
                  ae("Standard", -1)
                ])]),
                _: 1
              }),
              B(b(be), { intent: "primary" }, {
                default: te(() => [...P[14] || (P[14] = [
                  ae("Speichern", -1)
                ])]),
                _: 1
              }),
              B(b(be), { intent: "quiet" }, {
                default: te(() => [...P[15] || (P[15] = [
                  ae("Abbrechen", -1)
                ])]),
                _: 1
              }),
              B(b(be), { intent: "danger" }, {
                default: te(() => [...P[16] || (P[16] = [
                  ae("Löschen", -1)
                ])]),
                _: 1
              }),
              B(b(be), { disabled: "" }, {
                default: te(() => [...P[17] || (P[17] = [
                  ae("Gesperrt", -1)
                ])]),
                _: 1
              }),
              B(b(be), { busy: "" }, {
                default: te(() => [...P[18] || (P[18] = [
                  ae("Lädt", -1)
                ])]),
                _: 1
              })
            ...
            u("div", $y, [
              B(b(be), { size: "sm" }, {
                default: te(() => [...P[19] || (P[19] = [
                  ae("Klein", -1)
                ])]),
                _: 1
              }),
              B(b(be), { size: "md" }, {
                default: te(() => [...P[20] || (P[20] = [
                  ae("Mittel", -1)
                ])]),
                _: 1
              }),
              B(b(be), { size: "lg" }, {
                default: te(() => [...P[21] || (P[21] = [
                  ae("Groß", -1)
                ])]),
                _: 1
              })
          ...
          u("section", Ry, [
            ...
            u("div", Ey, [
              B(b(gn), {
                ...
                "onUpdate:modelValue": P[3] || (P[3] = (Q) => O.value.text = Q),
                label: "Titel"
              }, null, 8, ["modelValue"]),
              B(b(gn), {
                modelValue: O.value.number,
                "onUpdate:modelValue": P[4] || (P[4] = (Q) => O.value.number = Q),
                label: "Messwert",
                type: "number",
                suffix: "%"
              }, null, 8, ["modelValue"]),
              B(b(Un), {
                modelValue: O.value.choice,
                "onUpdate:modelValue": P[5] || (P[5] = (Q) => O.value.choice = Q),
                label: "Datenquelle",
                options: Y
              }, null, 8, ["modelValue"]),
              B(b(Zl), {
                modelValue: O.value.colour,
                "onUpdate:modelValue": P[6] || (P[6] = (Q) => O.value.colour = Q),
                label: "Farbe"
              }, null, 8, ["modelValue"]),
              B(b(x_), {
                modelValue: O.value.when,
                "onUpdate:modelValue": P[7] || (P[7] = (Q) => O.value.when = Q),
                label: "Stichtag"
              }, null, 8, ["modelValue"]),
              B(b(S_), {
                modelValue: O.value.amount,
                "onUpdate:modelValue": P[8] || (P[8] = (Q) => O.value.amount = Q),
                label: "Deckung",
                suffix: "%"
              }, null, 8, ["modelValue"]),
              B(b(gn), {
                modelValue: O.value.note,
                "onUpdate:modelValue": P[9] || (P[9] = (Q) => O.value.note = Q),
                label: "Notiz",
                rows: 2,
                stacked: ""
              }, null, 8, ["modelValue"]),
              B(b(gn), {
                modelValue: O.value.text,
                "onUpdate:modelValue": P[10] || (P[10] = (Q) => O.value.text = Q),
                label: "Mit Fehler",
          ...
          u("section", Ty, [
            ...
            u("div", Oy, [
              B(b(C_), {
                ...
                "onUpdate:modelValue": P[11] || (P[11] = (Q) => O.value.checked = Q),
                label: "Im Board zeigen"
              }, null, 8, ["modelValue"]),
              B(b(Jl), {
                modelValue: O.value.on,
                "onUpdate:modelValue": P[12] || (P[12] = (Q) => O.value.on = Q),
                label: "Automatisch aktualisieren"
              }, null, 8, ["modelValue"])
            ]),
            u("div", Ly, [
              B(b(qt), null, {
                default: te(() => [...P[24] || (P[24] = [
                  ae("neutral", -1)
                ])]),
                _: 1
              }),
              B(b(qt), { tone: "accent" }, {
                default: te(() => [...P[25] || (P[25] = [
                  ae("geladen", -1)
                ])]),
                _: 1
              }),
              B(b(qt), { tone: "ok" }, {
                default: te(() => [...P[26] || (P[26] = [
                  ae("im Rahmen", -1)
                ])]),
                _: 1
              }),
              B(b(qt), { tone: "warn" }, {
                default: te(() => [...P[27] || (P[27] = [
                  ae("prüfen", -1)
                ])]),
                _: 1
              }),
              B(b(qt), { tone: "err" }, {
                default: te(() => [...P[28] || (P[28] = [
                  ae("getrennt", -1)
                ])]),
                _: 1
              }),
              B(b(qt), {
                ...
                default: te(() => [...P[29] || (P[29] = [
                  ae("14", -1)
                ])]),
                _: 1
              })
            ]),
            B(b(A_), { label: "Trenner" })
          ])
        ])) : (_(), A("div", By, [
          u("aside", Dy, [
            (_(!0), A(Ce, null, Ge(b(h), (Q) => (_(), A("button", {
              key: Q.id,
              type: "button",
              class: Ee(["theme", { on: Q.id === b(o).id }]),
              onClick: (ve) => b(c)(Q.id)
            }, [
              u("span", Vy, [
                (_(!0), A(Ce, null, Ge($(Q), (ve, Ve) => (_(), A("i", {
                  key: Ve,
                  style: Yt({ background: ve })
                }, null, 4))), 128))
              ]),
              u("span", Py, z(Q.name), 1),
              u("span", My, z(Q.note), 1)
            ], 10, Wy))), 128))
          ]),
          u("section", Uy, [
            (_(!0), A(Ce, null, Ge(b(mi), (Q) => (_(), A("div", {
              key: Q.id,
              class: "group"
            }, [
              u("button", {
                type: "button",
                class: "group__head",
                "aria-expanded": K.value === Q.id,
                onClick: (ve) => K.value = K.value === Q.id ? "" : Q.id
              }, [
                u("span", Fy, z(K.value === Q.id ? "▾" : "▸"), 1),
                u("span", zy, z(Q.label), 1),
                u("span", qy, z(Q.tokens.length), 1)
              ], 8, Ny),
              K.value === Q.id ? (_(), A("div", Gy, [
                Q.note ? (_(), A("p", Hy, z(Q.note), 1)) : re("", !0),
                (_(!0), A(Ce, null, Ge(Q.tokens, (ve) => (_(), A("div", {
                  key: ve.name,
                  class: "token"
                }, [
                  D(ve) ? (_(), A("span", {
                    key: 0,
                    class: "token__swatch",
                    style: Yt({ background: b(m)(ve.name) }),
                    "aria-hidden": "true"
                  }, null, 4)) : (_(), A("span", Ky)),
                  u("span", Yy, [
                    u("code", Zy, "--" + z(ve.name), 1),
                    u("span", Jy, z(ve.role), 1)
                  ]),
                  D(ve) ? (_(), A("input", {
                    key: 2,
                    class: "token__picker",
                    ...
                    onInput: (Ve) => b(E)(ve.name, Ve.target.value)
                  }, null, 40, Xy)) : re("", !0),
                  u("input", {
                    class: "token__value",
                    type: "text",
                    ...
                    onChange: (Ve) => b(E)(ve.name, Ve.target.value)
                  }, null, 40, Qy),
                  u("button", {
                    class: "token__reset",
                    type: "button",
                    disabled: !b(I)(ve.name),
                    title: b(I)(ve.name) ? "Auf den Wert des Themas zurücksetzen" : "Unverändert",
                    onClick: (Ve) => b(C)(ve.name)
                  }, " ↺ ", 8, jy)
                ]))), 128))
              ])) : re("", !0)
            ]))), 128))
...
}), nw = /* @__PURE__ */ je(ew, [["__scopeId", "data-v-a84ca0bf"]]), It = tu.createLogger("daanse:system:actions");
class tw {
  constructor(h, o) {
  ...
  async changePage(h) {
    ...
    const o = new Promise((y) => {
      const m = (I) => {
        I.pageId === h && (It("📄 Received pageLoaded event for page: %s", h), this.eventBus.off("system:pageLoaded", m), y());
      };
      this.eventBus.on("system:pageLoaded", m), setTimeout(() => {
        this.eventBus.off("system:pageLoaded", m), It("⚠️ pageLoaded timeout for page: %s, continuing anyway", h), y();
      }, 5e3);
    });
...
async function rw(g, h, o) {
  ...
  const y = new tw(g, o);
  h.registerInstance("SystemActions", y, "SystemActions"), It("✅ System actions registered");
}
var iw = Object.defineProperty, ow = Object.getOwnPropertyDescriptor, aw = (g, h, o, y) => {
  for (var m = ow(h, o), I = g.length - 1, c; I >= 0; I--)
    (c = g[I]) && (m = c(h, o, m) || m);
  return m && iw(h, o, m), m;
};
const sw = tu.createLogger("daanse:system:actions");
class Xo {
  testAction(...h) {
...
aw([
  K_({ eventType: "test.console" })
], Xo.prototype, "testAction");
async function lw(g) {
  g.registerWidgetType("test", Xo, "system");
  const h = new Xo();
  g.registerInstance("test", h, "system"), sw("✅ Test actions registered");
}
let ot;
async function Ow({ services: g, log: h }) {
  Im(), tm(), sm(), ot = p_(Vm), ot.use(Gt), ot.provide("codeEditorType", "monaco");
  const o = g, y = (G) => {
    const O = g.get(G);
    ot?.provide(G, O), ot?.provide(Symbol.for(G), O);
  };
  for (const G of g.getServiceIds()) y(G);
  o.addListener?.({ onServiceEvent: (G) => y(G.serviceId) }), g.register(i_, ot);
  const m = g.getRequired(V_), I = g.getRequired(P_);
  if (m.getConnections().length === 0) {
    const G = m.createConnection("");
    G.uid = "test", G.name = "Test Connection 01", G.type = "rest", G.config = { url: "https://jsonplaceholder.typicode.com/" }, m.saveConnection(G);
    const O = I.createDatasource("");
    O.uid = "test_ds", O.name = "Test DataSource 01", O.type = "rest", O.connection = G, O.config = { resourceUrl: "posts" }, I.saveDatasource(O);
  }
  g.getRequired(a_).registerWrapperType({
    type: y_,
    create: (G) => new b_(G)
  });
  const c = g.getRequired(Kl), E = new Dl();
  E.path = "/configuration", E.name = "config", E.component = by, c.registerRoute(E);
  const C = new Dl();
  C.path = "/appearance", C.name = "appearance", C.component = nw, c.registerRoute(C);
  const f = g.getRequired(Hl), w = new v_();
  ...
  const K = c;
  for (const G of K.getAllRoutesArray?.() ?? [])
    Gt.addRoute({
      path: G.path,
      name: G.name,
  ...
  const q = window.location.pathname + window.location.search + window.location.hash;
  Gt.resolve(q).matched.length && Gt.currentRoute.value.fullPath !== q && Gt.replace(q);
  try {
    await rw(
      Gt,
      g.getRequired(Ll),
    ...
    h.error("system actions failed", G);
  }
  try {
    await lw(g.getRequired(Ll)), h.info("test actions registered");
  } catch (G) {
    h.error("test actions failed", G);
  }
  ot.mount("#app"), h.info("shell mounted");
}
function Lw({ services: g }) {
