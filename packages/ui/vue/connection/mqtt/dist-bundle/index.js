...
function activate({ services }) {
  services.register("MqttConnectionSettings", _sfc_main);
  services.getRequired(CONNECTION_REPOSITORY).registerConnectionType("mqtt", {
    Connection: MqttConnectionIdentifier,
    Settings: settingsSymbol
  });
}
function deactivate({ services }) {
  services.getRequired(CONNECTION_REPOSITORY).unregisterConnectionType("mqtt");
