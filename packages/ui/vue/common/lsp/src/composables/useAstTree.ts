...
import { getLanguageClient, type AstTreeNode } from 'org.eclipse.daanse.board.app.lib.lsp.core'
import {
  rangeContains,
  toFileUri,
  useEditorSession,
...
export interface AstTreeRow {
  /** Path from the root in AST mode, symbol position in outline mode. */
  id: string
  /** `slot: $type` for AST nodes, the symbol name in outline mode. */
  label: string
  /** Dimmed secondary text — a scalar's value, or a symbol's detail. */
  detail?: string
...
/** How deep a freshly loaded tree opens itself. */
const INITIAL_DEPTH = 3

function slotLabel(node: AstTreeNode): string | undefined {
  if (!node.property) return undefined
  return node.index === undefined ? node.property : `${node.property}[${node.index}]`
}

function toRow(node: AstTreeNode): AstTreeRow {
  const slot = slotLabel(node)
  const label =
    node.kind === 'node' ? (slot ? `${slot}: ${node.label}` : node.label) : (slot ?? node.label)
  return {
    id: node.id,
    label,
...
function symbolToRow(symbol: DocumentSymbol, id: string): AstTreeRow {
  return {
    ...
    children: (symbol.children ?? []).map((child, index) => symbolToRow(child, `${id}.${index}`))
  }
}

/**
 * Standard `textDocument/documentSymbol`, straight over the LSP client.
 *
 * The Monaco build went through `vscode.executeDocumentSymbolProvider`, which
 * needed the VS Code API shim to be running. There is no such shim here, and
 * there does not need to be: the request is plain LSP and the client already
 * speaks it.
 *
 * A server may answer with `SymbolInformation[]` instead, which is flat and
 * carries `location` rather than `range`; those are ignored rather than
 * rendered wrongly, since a flat list is what `hasOutline: false` exists to
 * suppress.
 */
async function loadOutline(session: EditorSession): Promise<AstTreeRow[]> {
  ...
  const symbols = await client.request<
    ...
    DocumentSymbol[] | null
  >('textDocument/documentSymbol', { textDocument: { uri: toFileUri(session.uri.value) } })
  return (symbols ?? [])
...
/**
 * The chain of rows containing a caret position, outermost first.
 *
 * Rows with no range — error recovery synthesises nodes without a CST node —
 * are transparent rather than blocking: their children are still searched.
 */
...
export const AST_TREE_MODEL: InjectionKey<AstTreeModel> = Symbol('olap-editor ast tree')
...
export function createAstTreeModel(session: EditorSession): AstTreeModel {
  ...
  /**
   * Outline mode needs a document-symbol provider that emits something worth
   * navigating by. The language package says whether it has one; Langium's
   * default emits a row per named node, which for some grammars is a flat list
   * of every identifier in the document and worse than no outline. The panel
   * hides the toggle rather than offering it, so this also guards the mode
   * itself against a stale value when the language switches.
   */
  const outlineAvailable = computed(() => session.language.value.hasOutline === true)

  let lastSeed = ''
  let outlineToken = 0

  /** Opens every row above the cap, so a fresh tree is not a single closed row. */
  ...
  /** Expansion and selection reset only when the document or the mode changes,
   * not on every re-parse — otherwise typing would collapse the tree. */
  ...
  watch([session.ast, mode, session.uri], () => void rebuild(), { immediate: true })
  ...
  return {
    ...
    /** Click: highlight the source, and open or close the row. */
...
/** Creates the model for the ambient session and shares it with descendants. */
...
export function useAstTree(): AstTreeModel {
  ...
  if (!model) {
    throw new Error('No AST tree model provided. Call provideAstTree() in a parent component.')
  }
  return model
}
