import { a8 as S, a3 as R, b2 as K, _ as d, g as Q, s as Y, a as tt, b as et, q as at, p as nt, l as z, c as rt, F as it, I as st, N as ot, e as lt, z as ct, G as ut } from "./index-EP6gW4O2.js";
import { p as dt } from "./chunk-4BX2VUAB-BI0NbGAY.js";
import { p as pt } from "./wardley-L42UT6IY-C-B6soca.js";
import { d as P } from "./arc-CzGDrSHx.js";
...
`, "getStyles"), kt = bt, Et = /* @__PURE__ */ d((t) => {
...
` + t);
  ...
  const u = s.getSections(), A = Et(u), C = [
    i.pie1,
    i.pie2,
    i.pie3,
    i.pie4,
  ...
  const G = A.filter((n) => (n.data.value / b * 100).toFixed(0) !== "0"), k = gt(C).domain([
    ...u.keys()
  ]);
  c.selectAll("mySlices").data(G).enter().append("path").attr("d", D).attr("fill", (n) => k(n.data.label)).attr("class", "pieCircle"), c.selectAll("mySlices").data(G).enter().append("text").text((n) => (n.data.value / b * 100).toFixed(0) + "%").attr("transform", (n) => "translate(" + $.centroid(n) + ")").style("text-anchor", "middle").attr("class", "slice");
  const V = c.append("text").text(s.getDiagramTitle()).attr("x", 0).attr("y", -400 / 2).attr("class", "pieTitleText"), N = [...u.entries()].map(([n, M]) => ({
    label: n,
    value: M
  })), E = c.selectAll(".legend").data(N).enter().append("g").attr("class", "legend").attr("transform", (n, M) => {
    const O = r + l, Z = O * N.length / 2, H = 12 * r, J = M * O - Z;
    return "translate(" + H + "," + J + ")";
  });
  E.append("rect").attr("width", r).attr("height", r).style("fill", (n) => k(n.label)).style("stroke", (n) => k(n.label)), E.append("text").attr("x", r + l).attr("y", r - l).text((n) => s.getShowData() ? `${n.label} [${n.value}]` : n.label);
  const U = Math.max(
    ...E.selectAll("text").nodes().map((n) => n?.getBoundingClientRect().width ?? 0)
