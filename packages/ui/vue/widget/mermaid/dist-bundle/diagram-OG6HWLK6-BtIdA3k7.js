import { _ as w, K as de, D as Q, F as Z, I as he, e as ue, l as J, bd as P, d as Y, b as pe, a as fe, p as me, q as ge, g as ye, s as Se, G as ve, be as xe, z as be } from "./index-3-rTiW2_.js";
import { s as we } from "./chunk-2J33WTMH-CjHBRsVM.js";
import { p as Ce } from "./chunk-4BX2VUAB-CXV_bOwN.js";
import { p as Te } from "./wardley-L42UT6IY-C-B6soca.js";
import { b as H } from "./defaultLocale-D7EN2tov.js";
import { o as U } from "./ordinal-DfAQgscy.js";
function Le(e) {
...
function Ue(e, a, n, l, r, o) {
  for (var d = [], h = a.children, m, c, u = 0, b = 0, s = h.length, x, S, v = a.value, p, g, M, N, z, R, V; u < s; ) {
    x = r - n, S = o - l;
    do
      p = h[b++].value;
    while (!p && b < s);
    for (g = M = p, R = Math.max(S / x, x / S) / (v * e), V = p * p * R, z = Math.max(M / V, V / g); b < s; ++b) {
...
function Je() {
  var e = Ze, a = !1, n = 1, l = 1, r = [0], o = O, d = O, h = O, m = O, c = O;
  function u(s) {
...
var Qe = /* @__PURE__ */ w((e, a) => {
  ...
  try {
    ...
    if (t === "$0,0")
      p = /* @__PURE__ */ w((i) => "$" + H(",")(i), "valueFormat");
    else if (t.startsWith("$") && t.includes(",")) {
      const i = /\.\d+/.exec(t), f = i ? i[0] : "";
      p = /* @__PURE__ */ w((C) => "$" + H("," + f)(C), "valueFormat");
    } else if (t.startsWith("$")) {
      const i = t.substring(1);
      p = /* @__PURE__ */ w((f) => "$" + H(i || "")(f), "valueFormat");
    } else
      p = H(t);
  } catch (t) {
    J.error("Error creating format function:", t), p = H(",");
  }
  const g = U().range([
    "transparent",
    c.cScale0,
  ...
  const z = b.append("g").attr("transform", `translate(0, ${u})`).attr("class", "treemapContainer"), R = ee(m).sum((t) => t.value ?? 0).sort((t, i) => (i.value ?? 0) - (t.value ?? 0)), te = Je().size([s, x]).paddingTop(
    (t) => t.children && t.children.length > 0 ? q + B : 0
  ).paddingInner(d).paddingLeft((t) => t.children && t.children.length > 0 ? B : 0).paddingRight((t) => t.children && t.children.length > 0 ? B : 0).paddingBottom((t) => t.children && t.children.length > 0 ? B : 0).round(!0)(R), re = te.descendants().filter((t) => t.children && t.children.length > 0), E = z.selectAll(".treemapSection").data(re).enter().append("g").attr("class", "treemapSection").attr("transform", (t) => `translate(${t.x0},${t.y0})`);
  ...
  }), o.showValues !== !1 && E.append("text").attr("class", "treemapSectionValue").attr("x", (t) => t.x1 - t.x0 - 10).attr("y", q / 2).attr("text-anchor", "end").attr("dominant-baseline", "middle").text((t) => t.value ? p(t.value) : "").attr("font-style", "italic").attr("style", (t) => {
    if (t.depth === 0)
      return "display: none;";
  ...
  X.append("rect").attr("width", (t) => t.x1 - t.x0).attr("height", (t) => t.y1 - t.y0).attr("class", "treemapLeaf").attr("fill", (t) => t.parent ? g(t.parent.data.name) : g(t.data.name)).attr("style", (t) => P({ cssCompiledStyles: t.data.cssCompiledStyles }).nodeStyles).attr("fill-opacity", 0.3).attr("stroke", (t) => t.parent ? g(t.parent.data.name) : g(t.data.name)).attr("stroke-width", 3), X.append("clipPath").attr("id", (t, i) => `clip-${a}-${i}`).append("rect").attr("width", (t) => Math.max(0, t.x1 - t.x0 - 4)).attr("height", (t) => Math.max(0, t.y1 - t.y0 - 4)), X.append("text").attr("class", "treemapLabel").attr("x", (t) => (t.x1 - t.x0) / 2).attr("y", (t) => (t.y1 - t.y0) / 2).attr("style", (t) => {
    ...
    for (; K > A && y > _ && (y--, I = Math.max(
      k,
      Math.min(F, Math.round(y * T))
    ), !(I < k && y === _)); )
      i.style("font-size", `${y}px`), K = y + W + I;
  ...
  const a = de(), n = Q(), l = Z(a, n.themeVariables), r = Z(rt, e), o = r.titleColor ?? l.titleColor, d = r.labelColor ?? l.textColor, h = r.valueColor ?? l.textColor;
  return `
  .treemapNode.section {
