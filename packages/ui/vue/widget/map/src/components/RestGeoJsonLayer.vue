...
interface RestGeoJsonLayerProps {
  layerData: any
  styleIds: string[]
  layerOptions: any
  markerPane: string
...
const props = defineProps<RestGeoJsonLayerProps>()
</script>

<template>
  <template v-for="styleID in styleIds" :key="styleID">
    <template v-if="layerData && layerData.features">
      <!-- Render non-point geometries (Polygons, LineStrings, etc.) -->
      <template v-for="feature in (filterFeatureCollection(layerData, getStyleById(styleID)!).features)" :key="'area-'+feature.id">
        <l-geo-json
          v-if="feature.geometry && !isPoint(feature.geometry)"
      ...
      <template v-for="feature in (filterFeatureCollection(layerData, getStyleById(styleID)!).features)" :key="'point-'+feature.id">
        <l-marker
